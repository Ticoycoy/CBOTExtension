...
function retryListener(listenerType, userId, retryFunction) {
  const key = `${listenerType}_${userId}`;
  const attempt = retryAttempts[key] || 0;
  if (attempt >= MAX_RETRY_ATTEMPTS) {
    console.error(`\u274C Max retry attempts reached for ${listenerType} listener`);
    delete retryAttempts[key];
    return;
  }
  const delay = RETRY_DELAYS[attempt] || RETRY_DELAYS[RETRY_DELAYS.length - 1];
  console.log(`\u{1F504} Retrying ${listenerType} listener in ${delay}ms (attempt ${attempt + 1}/${MAX_RETRY_ATTEMPTS})`);
  retryAttempts[key] = attempt + 1;
  retryTimeouts[key] = setTimeout(() => {
    console.log(`\u{1F504} Retrying ${listenerType} listener now...`);
    if (!auth.currentUser) {
      console.warn(`\u26A0\uFE0F User not authenticated during retry for ${listenerType}, skipping retry`);
      delete retryAttempts[key];
      return;
    }
    auth.currentUser.getIdToken(true).then((idToken) => {
      if (!idToken) {
        console.error(`\u274C No valid ID token during retry for ${listenerType}`);
        delete retryAttempts[key];
        return;
      }
      console.log(`\u2705 ID token refreshed for ${listenerType} retry`);
      retryFunction();
    }).catch((error) => {
      console.error(`\u274C Error refreshing ID token for ${listenerType} retry:`, error);
      delete retryAttempts[key];
    });
...
async function testFirestoreAccess() {
  console.log("\u{1F50D} Testing Firestore access and permissions...");
  if (!auth.currentUser) {
    console.error("\u274C No authenticated user for Firestore test");
    return { success: false, error: "No authenticated user" };
  }
  try {
    console.log("\u{1F50D} Verifying user authentication token...");
    const idToken = await auth.currentUser.getIdToken(true);
    if (!idToken) {
      console.error("\u274C No valid ID token found");
      return { success: false, error: "No valid ID token" };
    }
    console.log("\u2705 User has valid ID token");
    console.log("\u{1F50D} Testing basic Firestore connection...");
    try {
      const testRef = doc(db, "test", "connection");
      await getDoc(testRef);
      console.log("\u2705 Basic Firestore connection successful");
    } catch (testError) {
      console.log("\u26A0\uFE0F Basic Firestore test failed (expected):", testError.code);
    }
    console.log("\u{1F50D} Testing CampaignData access...");
    try {
      const campaignRef = doc(db, "CampaignData", auth.currentUser.uid);
      const campaignDoc = await getDoc(campaignRef);
      console.log("\u2705 CampaignData access successful:", campaignDoc.exists());
      if (campaignDoc.exists()) {
        const data = campaignDoc.data();
        console.log("\u{1F4CA} CampaignData content preview:", {
          hasCampaignData: !!data.campaignData,
          hasBase64Data: !!data.base64Data,
          hasCitations: !!data.citations,
          keys: Object.keys(data)
        });
      }
    } catch (campaignError) {
      console.error("\u274C CampaignData access failed:", campaignError);
      if (campaignError.code === "permission-denied") {
        return { success: false, error: "Permission denied for CampaignData", code: campaignError.code };
      }
      return { success: false, error: `CampaignData access failed: ${campaignError.message}`, code: campaignError.code };
    }
    console.log("\u{1F50D} Testing plan_subscribers access...");
    try {
      const planRef = doc(db, "plan_subscribers", auth.currentUser.uid);
      const planDoc = await getDoc(planRef);
      console.log("\u2705 plan_subscribers access successful:", planDoc.exists());
      if (planDoc.exists()) {
        const data = planDoc.data();
        console.log("\u{1F4CA} Plan data preview:", {
          status: data.status,
          hasPlans: !!data.plans,
      ...
      console.error("\u274C plan_subscribers access failed:", planError);
      if (planError.code === "permission-denied") {
        return { success: false, error: "Permission denied for plan_subscribers", code: planError.code };
      }
      return { success: false, error: `plan_subscribers access failed: ${planError.message}`, code: planError.code };
    }
    console.log("\u{1F50D} Testing users access...");
    try {
      const userRef = doc(db, "users", auth.currentUser.uid);
      const userDoc = await getDoc(userRef);
      console.log("\u2705 users access successful:", userDoc.exists());
      if (userDoc.exists()) {
        const data = userDoc.data();
        console.log("\u{1F4CA} User data preview:", {
          role: data.role,
          email: data.email,
      ...
      console.error("\u274C users access failed:", userError);
      if (userError.code === "permission-denied") {
        return { success: false, error: "Permission denied for users", code: userError.code };
      }
      return { success: false, error: `users access failed: ${userError.message}`, code: userError.code };
    }
    console.log("\u{1F50D} Testing automation collections access...");
    try {
      const collectionName = await getAutomationCollectionName();
      console.log(`\u{1F50D} Testing ${collectionName} collection access...`);
      const automationRef = collection(db, collectionName);
      const q2 = query(automationRef, where("status", "==", "active"));
      const querySnapshot = await getDocs(q2);
      console.log(`\u2705 ${collectionName} collection access successful: ${querySnapshot.size} documents found`);
    } catch (automationError) {
      console.error("\u274C Automation collection access failed:", automationError);
      if (automationError.code === "permission-denied") {
        return { success: false, error: `Permission denied for automation collection`, code: automationError.code };
      }
      return { success: false, error: `Automation collection access failed: ${automationError.message}`, code: automationError.code };
    }
    console.log("\u2705 All Firestore access tests passed successfully");
    return { success: true, message: "All Firestore access tests passed" };
  } catch (error) {
...
function handleAuthRecovery() {
  console.log("\u{1F504} Attempting authentication recovery...");
  if (!auth.currentUser) {
    console.warn("\u26A0\uFE0F No authenticated user for recovery");
    chrome.storage.local.set({ CampaignDataStatus: "no_user_for_recovery" });
    return;
  }
  if (unsubscribeCampaignData) {
    console.log("\u{1F504} Clearing existing CampaignData listener...");
    unsubscribeCampaignData();
    unsubscribeCampaignData = null;
  }
  if (unsubscribePlanSubscriber) {
    console.log("\u{1F504} Clearing existing plan_subscribers listener...");
    unsubscribePlanSubscriber();
    unsubscribePlanSubscriber = null;
  }
  if (unsubscribeUserRole) {
    console.log("\u{1F504} Clearing existing users listener...");
    unsubscribeUserRole();
  ...
  const attemptRecovery = () => {
    recoveryAttempts++;
    console.log(`\u{1F504} Authentication recovery attempt ${recoveryAttempts}/${maxRecoveryAttempts}`);
    auth.currentUser.getIdToken(true).then(async (idToken) => {
      if (!idToken) {
        console.error(`\u274C Failed to refresh ID token during recovery (attempt ${recoveryAttempts})`);
        if (recoveryAttempts < maxRecoveryAttempts) {
          console.log(`\u23F3 Retrying recovery in 3 seconds...`);
          setTimeout(attemptRecovery, 3e3);
        } else {
          chrome.storage.local.set({ CampaignDataStatus: "recovery_failed" });
        }
        return;
      }
      console.log("\u2705 ID token refreshed successfully during recovery");
      chrome.storage.local.set({
        CampaignDataStatus: "recovered",
        isLoggedIn: true,
        userId: auth.currentUser.uid
      });
      console.log("\u{1F50D} Testing Firestore access after recovery...");
      const testResult = await testFirestoreAccess();
      if (!testResult.success) {
        console.error("\u274C Firestore access test failed after recovery:", testResult.error);
        chrome.storage.local.set({ CampaignDataStatus: "firestore_access_failed" });
        if (testResult.code === "permission-denied" && recoveryAttempts < maxRecoveryAttempts) {
          console.log("\u{1F504} Permission denied, attempting another recovery...");
          setTimeout(attemptRecovery, 5e3);
        }
        return;
      }
      console.log("\u2705 Firestore access test passed after recovery");
      setTimeout(() => {
        console.log("\u{1F504} Re-setting up listeners after authentication recovery...");
        if (auth.currentUser) {
          console.log("\u{1F504} Setting up CampaignData listener...");
          listenToUserCampaignData(auth.currentUser.uid);
          setTimeout(() => {
            console.log("\u{1F504} Setting up plan_subscribers listener...");
            listenToUserPlanSubscriber(auth.currentUser.uid);
          }, 1e3);
          setTimeout(() => {
            console.log("\u{1F504} Setting up users listener...");
            listenToUserRole(auth.currentUser.uid);
          }, 2e3);
        }
      }, 2e3);
    }).catch((error) => {
      console.error(`\u274C Error during authentication recovery (attempt ${recoveryAttempts}):`, error);
      if (recoveryAttempts < maxRecoveryAttempts) {
        console.log(`\u23F3 Retrying recovery in 3 seconds...`);
        setTimeout(attemptRecovery, 3e3);
      } else {
...
var DB_NAME3 = "CB-PHAA-Extension";
var DB_VERSION3 = 1;
var IndexedDBManager = class {
  ...
  async initialize() {
    return new Promise((resolve, reject) => {
      const indexedDB2 = self.indexedDB || globalThis.indexedDB;
      if (!indexedDB2) {
      ...
      const request = indexedDB2.open(DB_NAME3, DB_VERSION3);
      request.onerror = () => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db2.objectStoreNames.contains("automation_tasks")) {
          const taskStore = db2.createObjectStore("automation_tasks", { keyPath: "id" });
          taskStore.createIndex("domainName", "domainName", { unique: false });
          taskStore.createIndex("status", "status", { unique: false });
          taskStore.createIndex("lastModified", "lastModified", { unique: false });
          console.log("\u{1F4E6} Created automation_tasks store");
        }
        if (!db2.objectStoreNames.contains("automation_versions")) {
          const versionStore = db2.createObjectStore("automation_versions", { keyPath: "domainName" });
          versionStore.createIndex("lastSync", "lastSync", { unique: false });
          console.log("\u{1F4E6} Created automation_versions store");
        }
        if (!db2.objectStoreNames.contains("usage_stats")) {
          const usageStore = db2.createObjectStore("usage_stats", { keyPath: "id" });
          usageStore.createIndex("userId", "userId", { unique: false });
          usageStore.createIndex("domainName", "domainName", { unique: false });
          usageStore.createIndex("date", "date", { unique: false });
          console.log("\u{1F4E6} Created usage_stats store");
        }
        if (!db2.objectStoreNames.contains("cache")) {
          const cacheStore = db2.createObjectStore("cache", { keyPath: "key" });
          cacheStore.createIndex("timestamp", "timestamp", { unique: false });
          cacheStore.createIndex("type", "type", { unique: false });
          console.log("\u{1F4E6} Created cache store");
        }
        if (!db2.objectStoreNames.contains("consolidation_setup")) {
          const consolidationStore = db2.createObjectStore("consolidation_setup", { keyPath: "domainName" });
          consolidationStore.createIndex("lastModified", "lastModified", { unique: false });
          console.log("\u{1F4E6} Created consolidation_setup store");
        }
      };
    });
  }
  // Automation Tasks CRUD Operations
  async saveAutomationTask(task) {
    return this.performTransaction("automation_tasks", "readwrite", (store) => {
      store.put(task);
    });
  }
  async getAutomationTask(id) {
    return this.performTransaction("automation_tasks", "readonly", (store) => {
      return store.get(id);
    });
  }
  async getAutomationTaskByDomain(domainName) {
    return this.performTransaction("automation_tasks", "readonly", (store) => {
      const index = store.index("domainName");
      return index.get(domainName);
    });
  }
  async getAllAutomationTasks() {
    return this.performTransaction("automation_tasks", "readonly", (store) => {
      return store.getAll();
    });
  }
  async getActiveAutomationTasks() {
    return this.performTransaction("automation_tasks", "readonly", (store) => {
  ...
  // Version Management
  async saveAutomationVersion(version4) {
    return this.performTransaction("automation_versions", "readwrite", (store) => {
      store.put(version4);
    });
  }
  async getAutomationVersion(domainName) {
    return this.performTransaction("automation_versions", "readonly", (store) => {
  ...
  async saveUsageStats(stats) {
    return this.performTransaction("usage_stats", "readwrite", (store) => {
      store.put(stats);
    });
  }
  async getUsageStats(userId, date) {
    return this.performTransaction("usage_stats", "readonly", (store) => {
      const index = store.index("userId");
      return index.get(IDBKeyRange.only(userId));
    });
  }
  ...
  // Cache Operations
  async setCache(key, data, type, ttl = 36e5) {
    const cacheData = {
      key,
      data,
      type,
    ...
    return this.performTransaction("cache", "readwrite", (store) => {
      store.put(cacheData);
    });
  }
  async getCache(key) {
    return this.performTransaction("cache", "readonly", (store) => {
  ...
  async clearAllAutomationCaches() {
    return new Promise((resolve, reject) => {
      if (!this.db) {
        console.warn("\u26A0\uFE0F [CACHE] IndexedDB not initialized, cannot clear caches");
        resolve();
        return;
      }
      ...
      request.onsuccess = (event) => {
        ...
        if (cursor) {
          ...
          if (key && key.startsWith("automation_") || cacheData.type === "automation") {
            cursor.delete();
            deletedCount++;
            console.log(`\u{1F5D1}\uFE0F [CACHE] Cleared cache for: ${key}`);
          }
          cursor.continue();
        } else {
          console.log(`\u2705 [CACHE] Cleared ${deletedCount} automation cache entries`);
          resolve();
        }
      };
      request.onerror = () => {
        console.error("\u274C [CACHE] Error clearing automation caches:", request.error);
        reject(request.error);
      };
      transaction.onerror = () => {
        console.error("\u274C [CACHE] Transaction error clearing automation caches:", transaction.error);
        reject(transaction.error);
      };
    });
  }
  // Consolidation Setup CRUD Operations
  async saveConsolidationSetup(setup) {
    if (!setup.domainName || typeof setup.domainName !== "string" || setup.domainName.trim() === "") {
      throw new Error(`Invalid domainName for consolidation setup: ${setup.domainName}`);
    }
    const normalizedSetup = {
      ...setup,
      domainName: setup.domainName.trim()
    };
    return this.performTransaction("consolidation_setup", "readwrite", (store) => {
      store.put(normalizedSetup);
    });
  }
  async getConsolidationSetup(domainName) {
    return this.performTransaction("consolidation_setup", "readonly", (store) => {
      return store.get(domainName);
    });
  }
  async getAllConsolidationSetups() {
    return this.performTransaction("consolidation_setup", "readonly", (store) => {
      return store.getAll();
    });
  }
  async deleteConsolidationSetup(domainName) {
    return this.performTransaction("consolidation_setup", "readwrite", (store) => {
...
var dbManager = new IndexedDBManager();
var AutomationSyncService = class {
  constructor() {
    this.lastSync = /* @__PURE__ */ new Map();
    this.syncInterval = 5 * 60 * 1e3;
    // 5 minutes (reduced from 30 minutes for faster updates)
    this.consolidationSyncInterval = 60 * 60 * 1e3;
    // 1 hour
    this.syncStatus = {
      lastSyncTime: null,
      lastConsolidationSyncTime: null,
      totalTasks: 0,
      activeTasks: 0,
      totalConsolidationSetups: 0,
      lastError: null,
  ...
  async initialize() {
    try {
      ...
      this.indexedDBAvailable = false;
      console.log("\u26A0\uFE0F Continuing without IndexedDB - will use Chrome Storage API");
      chrome.storage.local.set({ indexedDBAvailable: false }, () => {
        if (chrome.runtime.lastError) {
          console.error("\u274C Error setting IndexedDB flag:", chrome.runtime.lastError);
        }
      });
    }
    console.log("\u2705 Automation sync service initialized");
    if (this.indexedDBAvailable) {
      setInterval(() => this.syncAllAutomations(), this.syncInterval);
      setInterval(() => this.syncConsolidationSetups(), this.consolidationSyncInterval);
      await this.syncAllAutomations();
      await this.syncConsolidationSetups();
    } else {
      console.log("\u{1F504} Setting up Chrome Storage-based sync");
      setInterval(() => this.syncToChromeStorage(), this.syncInterval);
      await this.syncToChromeStorage();
    }
  }
  async syncAllAutomations() {
    if (!this.indexedDBAvailable) {
      console.log("\u23F3 IndexedDB not available - skipping sync");
      return;
    }
    ...
    const authCheck = await new Promise((resolve) => {
      chrome.storage.local.get(["isLoggedIn", "userId"], (result) => {
        ...
        const hasAuthUser = auth.currentUser !== null;
        const authUserId = auth.currentUser?.uid || null;
        const details = {
          storageIsLoggedIn: isLoggedIn,
          storageUserId: userId,
          authCurrentUser: hasAuthUser,
          authUserId
        };
        resolve({
          isAuthenticated: isLoggedIn || hasAuthUser,
    ...
    if (!authCheck.isAuthenticated || !authCheck.userId) {
      console.log("\u2139\uFE0F [SYNC] No authenticated user - skipping sync (this is normal when logged out)");
      console.log("\u2139\uFE0F [SYNC] Auth check details:", {
    ...
    console.log(`\u2705 [SYNC] User authenticated: ${authCheck.userId}`);
    try {
      if (auth.currentUser) {
        console.log("\u{1F504} [SYNC] Refreshing authentication token before sync...");
        await auth.currentUser.getIdToken(true);
        console.log("\u2705 [SYNC] Token refreshed successfully");
      } else {
        console.warn("\u26A0\uFE0F [SYNC] auth.currentUser is null, but user is logged in according to storage - proceeding anyway");
      }
    } catch (tokenError) {
      console.error("\u274C [SYNC] Failed to refresh token:", tokenError);
    }
    this.syncStatus.syncInProgress = true;
    this.syncStatus.lastError = null;
    const syncStartTime = /* @__PURE__ */ new Date();
    try {
      console.log("\u{1F504} Starting automation sync...");
      const collectionName = await getAutomationCollectionName();
      console.log(`\u{1F4DA} Syncing from collection: ${collectionName}`);
      console.log(`\u{1F3AD} Role-based collection selection: ${collectionName} (user role determines this)`);
      let userRole = await new Promise((resolve) => {
        chrome.storage.local.get(["user_roles"], (result) => {
          const role = result.user_roles?.role || "user";
          console.log(`\u{1F3AD} [SYNC] Detected user role: "${role}"`);
          console.log(`\u{1F3AD} [SYNC] Full user_roles object:`, result.user_roles);
          resolve(role);
        });
      });
      if (userRole === "user" && collectionName === "automationTasks") {
        console.log(`\u26A0\uFE0F [SYNC] Role is "user" but collection is "automationTasks" - checking Firestore directly...`);
        try {
          if (authCheck.userId) {
            const userRef = doc(db, "users", authCheck.userId);
            const userDoc = await getDoc(userRef);
            if (userDoc.exists()) {
              const userData = userDoc.data();
              const firestoreRole = userData.role || "user";
              console.log(`\u2705 [SYNC] Found role in Firestore: "${firestoreRole}"`);
              if (firestoreRole === "admin" || firestoreRole === "collaborator") {
          ...
          console.warn(`\u26A0\uFE0F [SYNC] Could not fetch role from Firestore:`, roleError);
        }
      }
      const automationTasksRef = collection(db, collectionName);
      let querySnapshot;
      if (userRole === "admin") {
        console.log(`\u{1F4DA} [ADMIN] Fetching ALL automation tasks from collection: ${collectionName} (no status filter)`);
        console.log(`\u{1F4DA} [ADMIN] Admin role detected - will fetch ALL statuses`);
        const q2 = query(automationTasksRef);
        querySnapshot = await getDocs(q2);
      } else {
        console.log(`\u{1F4DA} Fetching automation tasks from collection: ${collectionName}`);
        console.log(`\u{1F50D} Filtering by status: active, completed, or hold (for ${userRole} role)`);
        const statusesToFetch = ["active", "completed", "hold"];
        const q2 = query(automationTasksRef, where("status", "in", statusesToFetch));
        querySnapshot = await getDocs(q2);
      }
      const statusCounts = {};
      const allStatuses = [];
      ...
      querySnapshot.docs.forEach((doc2) => {
        ...
        tasksByStatus[status].push({
          id: doc2.id,
          domainName: data.domainName || "unknown"
        });
      });
      console.log(`\u{1F4CA} [SYNC] Status distribution in Firestore (filtered):`, statusCounts);
      console.log(`\u{1F4CA} [SYNC] All statuses found:`, allStatuses);
      console.log(`\u{1F4CA} [SYNC] Tasks by status:`, tasksByStatus);
      if (userRole === "admin") {
        console.log(`\u{1F4CA} [ADMIN] Total tasks found (ALL statuses): ${querySnapshot.size}`);
      } else {
        console.log(`\u{1F4CA} [SYNC] Total tasks found (active/completed/hold): ${querySnapshot.size}`);
      }
      if (userRole !== "admin") {
        if (!statusCounts["completed"] && !statusCounts["hold"]) {
          console.warn(`\u26A0\uFE0F [SYNC] WARNING: No 'completed' or 'hold' tasks found! Only found:`, Object.keys(statusCounts));
        } else {
          console.log(`\u2705 [SYNC] Found tasks with statuses:`, Object.keys(statusCounts));
        }
      } else {
        console.log(`\u2705 [ADMIN] Found tasks with all statuses:`, Object.keys(statusCounts));
      }
      let syncedCount = 0;
      let totalTasks = querySnapshot.size;
      if (userRole === "admin") {
        console.log(`\u{1F4CA} [ADMIN] Found ${totalTasks} automation tasks in Firestore (ALL statuses)`);
      } else {
        console.log(`\u{1F4CA} Found ${totalTasks} automation tasks in Firestore (status: active/completed/hold)`);
      }
      const domainsFound = querySnapshot.docs.map((doc2) => {
        const data = doc2.data();
        return {
          id: doc2.id,
          ...
          controllerSteps: data.controller?.steps?.length || 0
        };
      });
      console.log("\u{1F50D} Domains found in Firestore:", domainsFound);
      console.log("\u{1F50D} Domain names list:", domainsFound.map((d) => d.domainName));
      const yplocalFound = domainsFound.find((d) => d.domainName === "yplocal.com");
      if (yplocalFound) {
        console.log(`\u2705 [SYNC] yplocal.com found in Firestore query:`, yplocalFound);
      } else {
        console.warn(`\u26A0\uFE0F [SYNC] yplocal.com NOT found in Firestore query results!`);
        console.warn(`\u26A0\uFE0F [SYNC] Searched collection: ${collectionName}`);
        console.warn(`\u26A0\uFE0F [SYNC] Query status filter: ${querySnapshot.empty ? "all" : querySnapshot.docs[0]?.data()?.status || "unknown"}`);
      }
      for (const doc2 of querySnapshot.docs) {
        const taskData = doc2.data();
        console.log(`\u{1F504} Processing automation for domain: ${taskData.domainName}`);
        console.log(`\u{1F4CB} Document data for ${taskData.domainName}:`, {
          id: doc2.id,
          domainName: taskData.domainName,
          status: taskData.status,
          stepsType: typeof taskData.steps,
          stepsLength: taskData.steps?.length || 0,
          hasSteps: !!taskData.steps,
          stepsPreview: taskData.steps ? taskData.steps.slice(0, 2) : "No steps",
          controllerStepsType: typeof taskData.controller?.steps,
          controllerStepsLength: taskData.controller?.steps?.length || 0,
          hasControllerSteps: !!taskData.controller?.steps,
          controllerStepsPreview: taskData.controller?.steps ? taskData.controller.steps.slice(0, 2) : "No controller steps"
        });
        if (taskData.steps && taskData.steps.length > 0) {
          console.log(`\u{1F50D} All step selectors for ${taskData.domainName} (${taskData.steps.length} steps):`);
          taskData.steps.forEach((step, index) => {
            console.log(`   Step ${index + 1}: ${step.action} - "${step.selector}" (${step.label || "No label"})`);
          });
        } else if (taskData.controller?.steps && taskData.controller.steps.length > 0) {
          console.log(`\u{1F50D} All controller step selectors for ${taskData.domainName} (${taskData.controller.steps.length} steps):`);
          taskData.controller.steps.forEach((step, index) => {
            console.log(`   Step ${index + 1}: ${step.action} - "${step.selector}" (${step.label || "No label"})`);
          });
        } else {
          console.log(`\u26A0\uFE0F No steps found for ${taskData.domainName} in either root or controller`);
        }
        let steps = [];
        if (taskData.steps && Array.isArray(taskData.steps) && taskData.steps.length > 0) {
          steps = taskData.steps;
          console.log(`\u{1F4CB} [SYNC] Using root steps array (${steps.length} steps) for ${taskData.domainName}`);
        } else if (taskData.controller?.steps && Array.isArray(taskData.controller.steps) && taskData.controller.steps.length > 0) {
          steps = taskData.controller.steps;
          console.log(`\u{1F4CB} [SYNC] Using controller steps array (${steps.length} steps) for ${taskData.domainName}`);
        } else {
          steps = [];
          console.warn(`\u26A0\uFE0F [SYNC] No steps found in root or controller for ${taskData.domainName}`);
        }
        if (taskData.steps && taskData.controller?.steps && Array.isArray(taskData.steps) && Array.isArray(taskData.controller.steps) && taskData.steps.length > 0 && taskData.controller.steps.length > 0) {
          console.warn(`\u26A0\uFE0F [SYNC] Both root and controller have steps for ${taskData.domainName}. Using root (${taskData.steps.length} steps). Controller has ${taskData.controller.steps.length} steps.`);
        }
        const automationTask = {
          id: doc2.id,
          assignPerson: taskData.assignPerson,
          controller: taskData.controller || {},
          settings: {
            maxRetries: taskData.settings?.maxRetries || taskData.controller?.settings?.maxRetries || 3,
            notifyOnComplete: taskData.settings?.notifyOnComplete || taskData.controller?.settings?.notifyOnComplete || true,
            screenshotOnError: taskData.settings?.screenshotOnError || taskData.controller?.settings?.screenshotOnError || true,
            timeout: taskData.settings?.timeout || taskData.controller?.settings?.timeout || 3e4
          },
          steps,
          createdAt: taskData.createdAt?.toDate() || /* @__PURE__ */ new Date(),
          createdBy: taskData.createdBy,
          domainName: taskData.domainName,
          lastModified: taskData.lastModified?.toDate() || /* @__PURE__ */ new Date(),
        ...
        console.log(`\u{1F4BE} Saving automation to IndexedDB: ${automationTask.domainName} (${automationTask.steps.length} steps)`);
        console.log(`\u{1F39B}\uFE0F [SYNC] Controller requiredCategory: ${automationTask.controller?.requiredCategory !== false ? "true (default)" : "false"}`);
        console.log(`\u{1F39B}\uFE0F [SYNC] Controller object keys:`, automationTask.controller ? Object.keys(automationTask.controller) : "no controller");
        if (!automationTask.id || !automationTask.domainName) {
          console.error(`\u274C [SYNC] Skipping invalid automation task:`, {
            id: automationTask.id,
            domainName: automationTask.domainName,
            hasId: !!automationTask.id,
            hasDomainName: !!automationTask.domainName
          });
          continue;
        }
        console.log(`\u{1F4CB} [SYNC] ALL ${automationTask.steps.length} steps for ${automationTask.domainName} BEFORE saving:`);
        automationTask.steps.forEach((step, index) => {
          console.log(`   Step ${index + 1}: action="${step.action}", selector="${step.selector}", id="${step.id}", label="${step.label || ""}"`);
        });
        if (automationTask.steps.length > 0) {
          console.log(`\u{1F50D} First 3 step selectors for ${automationTask.domainName}:`);
          automationTask.steps.slice(0, 3).forEach((step, index) => {
            console.log(`   Step ${index + 1}: ${step.action} - "${step.selector}"`);
          });
        }
        try {
          console.log(`\u{1F4BE} [SYNC] Saving task: ${automationTask.domainName} | Status: ${automationTask.status} | ID: ${automationTask.id} | Steps: ${automationTask.steps.length}`);
          await dbManager.saveAutomationTask(automationTask);
          const savedTask = await dbManager.getAutomationTaskByDomain(automationTask.domainName);
          if (savedTask) {
            console.log(`\u2705 [SYNC] Saved automation task to IndexedDB: ${automationTask.domainName} | Status: ${automationTask.status} | ID: ${automationTask.id}`);
            console.log(`\u{1F50D} [SYNC] Verification: IndexedDB now has ${savedTask.steps?.length || 0} steps (expected ${automationTask.steps.length})`);
            if (savedTask.steps?.length !== automationTask.steps.length) {
              console.error(`\u274C [SYNC] STEP COUNT MISMATCH! Saved ${automationTask.steps.length} but IndexedDB has ${savedTask.steps?.length || 0}`);
              console.error(`\u274C [SYNC] Original steps:`, automationTask.steps.map((s) => ({ action: s.action, selector: s.selector, id: s.id })));
              console.error(`\u274C [SYNC] Saved steps:`, savedTask.steps?.map((s) => ({ action: s.action, selector: s.selector, id: s.id })) || []);
            } else {
              console.log(`\u2705 [SYNC] Step count verified: ${savedTask.steps.length} steps saved correctly`);
            }
          } else {
            console.error(`\u274C [SYNC] Failed to verify saved task - could not read back from IndexedDB`);
          }
          const version4 = {
            domainName: automationTask.domainName,
            version: automationTask.lastModified.getTime().toString(),
            lastModified: automationTask.lastModified,
            lastSync: /* @__PURE__ */ new Date(),
            stepCount: automationTask.steps.length
          };
          try {
            await dbManager.saveAutomationVersion(version4);
            console.log(`\u2705 [SYNC] Saved automation version: ${automationTask.domainName}`);
          } catch (versionError) {
            console.error(`\u274C [SYNC] Failed to save version for ${automationTask.domainName}:`, versionError);
          }
          syncedCount++;
          console.log(`\u2705 Synced ${syncedCount}/${totalTasks}: ${automationTask.domainName} | Status: ${automationTask.status} | Steps: ${automationTask.steps.length}`);
        } catch (saveError) {
          console.error(`\u274C [SYNC] Failed to save automation task for ${automationTask.domainName}:`, saveError);
      ...
      this.syncStatus.lastSyncTime = /* @__PURE__ */ new Date();
      this.syncStatus.totalTasks = totalTasks;
      this.syncStatus.activeTasks = syncedCount;
      console.log("\u{1F5D1}\uFE0F [SYNC] Clearing automation caches to ensure fresh data...");
      try {
        await dbManager.clearAllAutomationCaches();
        console.log("\u2705 [SYNC] All automation caches cleared successfully");
      } catch (cacheError) {
        console.warn("\u26A0\uFE0F [SYNC] Failed to clear automation caches:", cacheError);
      }
      console.log("\u{1F504} [SYNC] Immediately syncing to Chrome Storage after IndexedDB update...");
      try {
        await this.syncToChromeStorage();
        console.log("\u2705 [SYNC] Chrome Storage sync completed immediately after IndexedDB sync");
      } catch (chromeStorageError) {
        console.error("\u274C [SYNC] Failed to sync to Chrome Storage immediately:", chromeStorageError);
      }
      ...
      if (this.syncStatus.syncHistory.length > 10) {
        this.syncStatus.syncHistory = this.syncStatus.syncHistory.slice(-10);
      }
      console.log(`\u2705 Sync completed: ${syncedCount}/${totalTasks} tasks synced successfully`);
      console.log(`\u{1F4CA} Sync Status:`, this.syncStatus);
    } catch (error) {
      this.syncStatus.lastError = error instanceof Error ? error.message : String(error);
      if (error.code === "permission-denied" || error.message?.includes("permission")) {
        console.warn("\u26A0\uFE0F [SYNC] Permission denied error - attempting token refresh and retry...");
        try {
          if (auth.currentUser) {
            await auth.currentUser.getIdToken(true);
            console.log("\u2705 [SYNC] Token refreshed, but retry not implemented (would require refactoring)");
          }
        } catch (refreshError) {
          console.error("\u274C [SYNC] Failed to refresh token after permission error:", refreshError);
        }
      }
      this.syncStatus.syncHistory.push({
        timestamp: syncStartTime,
  ...
  async syncConsolidationSetups() {
    if (!this.indexedDBAvailable) {
      console.log("\u23F3 IndexedDB not available - skipping consolidation setup sync");
      return;
    }
    const authCheck = await new Promise((resolve) => {
      chrome.storage.local.get(["isLoggedIn", "userId"], (result) => {
    ...
    if (!authCheck.isAuthenticated || !authCheck.userId) {
      console.log("\u2139\uFE0F [CONSOLIDATION_SYNC] No authenticated user - skipping sync (this is normal when logged out)");
      console.log("\u2139\uFE0F [CONSOLIDATION_SYNC] Auth check details:", {
        isAuthenticated: authCheck.isAuthenticated,
        userId: authCheck.userId,
        details: authCheck.details
      });
    ...
    try {
      if (auth.currentUser) {
        console.log("\u{1F504} [CONSOLIDATION_SYNC] Refreshing authentication token before sync...");
        await auth.currentUser.getIdToken(true);
        console.log("\u2705 [CONSOLIDATION_SYNC] Token refreshed successfully");
      } else {
        console.warn("\u26A0\uFE0F [CONSOLIDATION_SYNC] auth.currentUser is null, but user is logged in - proceeding anyway");
      }
    } catch (tokenError) {
      console.error("\u274C [CONSOLIDATION_SYNC] Failed to refresh token:", tokenError);
    }
    try {
      console.log("\u{1F504} Starting consolidation setup sync...");
      const consolidationSetupRef = collection(db, "consolidationSetup");
      const querySnapshot = await getDocs(consolidationSetupRef);
      let syncedCount = 0;
      const totalSetups = querySnapshot.size;
      console.log(`\u{1F4CA} Found ${totalSetups} consolidation setups in Firestore`);
      for (const doc2 of querySnapshot.docs) {
        const setupData = doc2.data();
        if (!setupData.domainName || typeof setupData.domainName !== "string" || setupData.domainName.trim() === "") {
          console.warn(`\u26A0\uFE0F Skipping consolidation setup ${doc2.id}: Invalid or missing domainName (${setupData.domainName})`);
          continue;
        }
        console.log(`\u{1F504} Processing consolidation setup for domain: ${setupData.domainName}`);
        const consolidationSetup = {
          id: doc2.id,
          domainName: setupData.domainName.trim(),
          // Ensure no leading/trailing whitespace
          config: setupData.config || {},
          lastModified: setupData.lastModified?.toDate() || /* @__PURE__ */ new Date(),
          createdAt: setupData.createdAt?.toDate() || /* @__PURE__ */ new Date()
        };
        try {
          console.log(`\u{1F4BE} Saving consolidation setup to IndexedDB: ${consolidationSetup.domainName}`);
          await dbManager.saveConsolidationSetup(consolidationSetup);
          syncedCount++;
          console.log(`\u2705 Synced ${syncedCount}/${totalSetups}: ${consolidationSetup.domainName}`);
        } catch (error) {
          console.error(`\u274C Error saving consolidation setup for ${consolidationSetup.domainName}:`, error);
          console.error(`\u274C Error details:`, {
            message: error.message,
            name: error.name,
            setupId: doc2.id,
            domainName: consolidationSetup.domainName
          });
        }
      }
      this.syncStatus.lastConsolidationSyncTime = /* @__PURE__ */ new Date();
      this.syncStatus.totalConsolidationSetups = totalSetups;
      console.log(`\u2705 Consolidation setup sync completed: ${syncedCount}/${totalSetups} setups synced successfully`);
    } catch (error) {
      this.syncStatus.lastError = error instanceof Error ? error.message : String(error);
      if (error.code === "permission-denied" || error.message?.includes("permission")) {
        console.warn("\u26A0\uFE0F [CONSOLIDATION_SYNC] Permission denied error - token may need refresh");
        try {
          if (auth.currentUser) {
            await auth.currentUser.getIdToken(true);
            console.log("\u2705 [CONSOLIDATION_SYNC] Token refreshed for next attempt");
          }
        } catch (refreshError) {
          console.error("\u274C [CONSOLIDATION_SYNC] Failed to refresh token after permission error:", refreshError);
        }
      }
      console.error("\u274C Error syncing consolidation setups:", error);
      console.error("\u274C Error details:", {
        code: error.code,
        message: error.message,
        stack: error.stack
  ...
  async getAutomationForDomain(domainName) {
    try {
      console.log(`\u{1F50D} Looking for automation for domain: ${domainName}`);
      if (!this.indexedDBAvailable) {
        console.log(`\u{1F4E1} IndexedDB not available - checking Chrome Storage first for ${domainName}`);
        const storageData = await new Promise((resolve) => {
          chrome.storage.local.get(["automation_tasks"], (result) => {
            resolve(result);
          });
        });
        if (storageData.automation_tasks) {
          const task2 = storageData.automation_tasks.find((t) => t.domainName === domainName);
          if (task2) {
            console.log(`\u2705 Found automation in Chrome Storage for ${domainName}:`, {
              id: task2.id,
              domainName: task2.domainName,
              stepCount: task2.steps?.length || 0,
              status: task2.status
            });
            return task2;
          }
        }
        console.log(`\u{1F4E1} Not found in Chrome Storage, fetching directly from Firestore for ${domainName}`);
        return await this.getAutomationFromFirestore(domainName);
      }
      console.log(`\u{1F50D} Looking for automation in IndexedDB for domain: ${domainName}`);
      try {
        await dbManager.clearCache(`automation_${domainName}`);
        console.log(`\u{1F5D1}\uFE0F [CACHE] Cleared IndexedDB cache for ${domainName} to ensure fresh data`);
      } catch (cacheError) {
      }
      chrome.storage.local.get(["automation_tasks"], (result) => {
        if (result.automation_tasks) {
          const hasStaleData = result.automation_tasks.some((t) => t.domainName === domainName);
          if (hasStaleData) {
            console.log(`\u{1F5D1}\uFE0F [CHROME_STORAGE] Detected potential stale data in Chrome Storage for ${domainName} - will be refreshed after IndexedDB read`);
          }
        }
      });
      console.log(`\u{1F4BE} Querying IndexedDB for domain: ${domainName}`);
      const task = await dbManager.getAutomationTaskByDomain(domainName);
      if (!task) {
        console.log(`\u{1F50D} Domain "${domainName}" not found in IndexedDB. Checking available domains...`);
        try {
          const allTasks = await dbManager.getAllAutomationTasks();
          const availableDomains = allTasks.map((t) => t.domainName);
          console.log(`\u{1F4CB} Available domains in IndexedDB:`, availableDomains);
          const similarDomains = availableDomains.filter(
            (domain) => domain.includes(domainName) || domainName.includes(domain) || domain.replace(/[^a-zA-Z0-9]/g, "") === domainName.replace(/[^a-zA-Z0-9]/g, "")
          );
          if (similarDomains.length > 0) {
            console.log(`\u{1F50D} Found similar domains:`, similarDomains);
            console.log(`\u{1F4A1} The requested domain "${domainName}" might be "${similarDomains[0]}"`);
          }
        } catch (err) {
          console.log(`\u26A0\uFE0F Could not retrieve all tasks for debugging:`, err);
        }
      }
      if (task) {
        console.log(`\u2705 Found automation in IndexedDB for ${domainName}:`, {
          id: task.id,
          domainName: task.domainName,
          stepCount: task.steps?.length || 0,
          status: task.status
        });
        console.log(`\u{1F39B}\uFE0F [INDEXEDDB] Controller requiredCategory: ${task.controller?.requiredCategory !== false ? "true (default)" : "false"}`);
        console.log(`\u{1F39B}\uFE0F [INDEXEDDB] Controller object keys:`, task.controller ? Object.keys(task.controller) : "no controller");
        if (task.steps && task.steps.length > 0) {
          console.log(`\u{1F4CB} [GET_AUTOMATION] ALL ${task.steps.length} steps retrieved from IndexedDB for ${domainName}:`);
          task.steps.forEach((step, index) => {
            console.log(`   Step ${index + 1}: action="${step.action}", selector="${step.selector || "undefined"}", id="${step.id}", label="${step.label || ""}"`);
          });
        } else {
          console.warn(`\u26A0\uFE0F [GET_AUTOMATION] No steps found in IndexedDB for ${domainName}`);
        }
        return task;
      } else {
        console.log(`\u274C No automation found in IndexedDB for domain: ${domainName}`);
        try {
          const allTasks = await dbManager.getAllAutomationTasks();
          console.log(`\u{1F4CA} All automations in IndexedDB:`, allTasks.map((t) => ({
            id: t.id,
      ...
      console.error(`\u274C Error loading automation for ${domainName}:`, error);
      console.log(`\u{1F504} Falling back to Firestore for ${domainName}`);
      return await this.getAutomationFromFirestore(domainName);
    }
  }
  // New method to get automation directly from Firestore
  async getAutomationFromFirestore(domainName) {
    try {
      console.log(`\u{1F4E1} Fetching automation directly from Firestore for ${domainName}`);
      const collectionName = await getAutomationCollectionName();
      console.log(`\u{1F50D} Collection: ${collectionName}`);
      console.log(`\u{1F3AD} Role-based collection selection: ${collectionName} (user role determines this)`);
      console.log(`\u{1F50D} Query conditions: domainName == "${domainName}" AND status == "active"`);
      const automationTasksRef = collection(db, collectionName);
      console.log(`\u{1F50D} Checking all documents in ${collectionName} collection...`);
      const allDocsQuery = query(automationTasksRef);
      const allDocsSnapshot = await getDocs(allDocsQuery);
      console.log(`\u{1F4CA} Total documents in ${collectionName} collection: ${allDocsSnapshot.size}`);
      if (allDocsSnapshot.size > 0) {
        const allDocs = allDocsSnapshot.docs.map((doc3) => {
        ...
        console.log(`\u{1F4CB} All documents in collection:`, allDocs);
      } else {
        console.log(`\u274C No documents found in ${collectionName} collection`);
      }
      const userRole = await new Promise((resolve) => {
        chrome.storage.local.get(["user_roles"], (result) => {
          resolve(result.user_roles?.role || "user");
        });
      });
      let querySnapshot;
      if (userRole === "admin") {
        console.log(`\u{1F4DA} [ADMIN] Fetching automation for domain "${domainName}" (no status filter)`);
        const q2 = query(
          automationTasksRef,
          where("domainName", "==", domainName)
        );
        querySnapshot = await getDocs(q2);
        console.log(`\u{1F4CA} [ADMIN] Query results for domain "${domainName}" (ALL statuses): ${querySnapshot.size} documents`);
      } else {
        const statusesToCheck = ["active", "completed", "hold"];
        const q2 = query(
          automationTasksRef,
          where("domainName", "==", domainName),
        ...
        querySnapshot = await getDocs(q2);
        console.log(`\u{1F4CA} Query results for domain "${domainName}" with status (active/completed/hold): ${querySnapshot.size} documents`);
      }
      if (querySnapshot.empty) {
        if (userRole === "admin") {
          console.log(`\u274C No automation found in Firestore for domain: ${domainName}`);
          return null;
        } else {
          console.log(`\u274C No automation found in Firestore for domain: ${domainName} with status (active/completed/hold)`);
          console.log(`\u{1F50D} Trying query without status filter to check if domain exists...`);
          const domainOnlyQuery = query(
            automationTasksRef,
            where("domainName", "==", domainName)
          );
          const domainOnlySnapshot = await getDocs(domainOnlyQuery);
          console.log(`\u{1F4CA} Documents with domain "${domainName}" (any status): ${domainOnlySnapshot.size}`);
          if (domainOnlySnapshot.size > 0) {
            const statuses = domainOnlySnapshot.docs.map((doc3) => doc3.data().status || "unknown");
            console.log(`\u26A0\uFE0F Domain exists but with different statuses:`, statuses);
            console.log(`\u26A0\uFE0F Only syncing tasks with status: active, completed, or hold`);
            const domainDocs = domainOnlySnapshot.docs.map((doc3) => {
              const data = doc3.data();
              return {
                id: doc3.id,
                domainName: data.domainName,
            ...
            console.log(`\u{1F4CB} Documents with domain "${domainName}":`, domainDocs);
            const acceptableStatusDoc = domainOnlySnapshot.docs.find((doc3) => {
              const status = doc3.data().status;
              return status === "active" || status === "completed" || status === "hold";
            });
            if (!acceptableStatusDoc) {
              console.log(`\u274C Domain "${domainName}" exists but has no acceptable status (active/completed/hold)`);
              return null;
            }
            const firstDoc = acceptableStatusDoc;
            const taskData2 = firstDoc.data();
            console.log(`\u2705 Found document with acceptable status "${taskData2.status}" - using it`);
            const automationTask2 = {
              id: firstDoc.id,
              assignPerson: taskData2.assignPerson,
              controller: taskData2.controller || {},
              settings: {
                maxRetries: taskData2.settings?.maxRetries || taskData2.controller?.settings?.maxRetries || 3,
                notifyOnComplete: taskData2.settings?.notifyOnComplete || taskData2.controller?.settings?.notifyOnComplete || true,
                screenshotOnError: taskData2.settings?.screenshotOnError || taskData2.controller?.settings?.screenshotOnError || true,
                timeout: taskData2.settings?.timeout || taskData2.controller?.settings?.timeout || 3e4
              },
              steps: taskData2.steps || taskData2.controller?.steps || [],
              createdAt: taskData2.createdAt?.toDate() || /* @__PURE__ */ new Date(),
              createdBy: taskData2.createdBy,
              domainName: taskData2.domainName,
              lastModified: taskData2.lastModified?.toDate() || /* @__PURE__ */ new Date(),
              lastModifiedBy: taskData2.lastModifiedBy,
              notes: taskData2.notes || "",
              priority: taskData2.priority || "medium",
              status: taskData2.status,
              timeline: taskData2.timeline || ""
            };
            console.log(`\u2705 Using automation with status "${taskData2.status}" for ${domainName}:`, {
              id: automationTask2.id,
              domainName: automationTask2.domainName,
              stepCount: automationTask2.steps?.length || 0,
              status: automationTask2.status
            });
            return automationTask2;
          }
          return null;
        }
      }
      ...
      const automationTask = {
        ...
        steps: taskData.steps || taskData.controller?.steps || [],
        createdAt: taskData.createdAt?.toDate() || /* @__PURE__ */ new Date(),
        createdBy: taskData.createdBy,
        domainName: taskData.domainName,
        lastModified: taskData.lastModified?.toDate() || /* @__PURE__ */ new Date(),
      ...
      console.log(`\u2705 Found automation in Firestore for ${domainName}:`, {
        id: automationTask.id,
        domainName: automationTask.domainName,
        stepCount: automationTask.steps?.length || 0,
        status: automationTask.status
      });
      console.log(`\u{1F39B}\uFE0F [FIRESTORE] Controller requiredCategory: ${automationTask.controller?.requiredCategory !== false ? "true (default)" : "false"}`);
      console.log(`\u{1F39B}\uFE0F [FIRESTORE] Controller object:`, automationTask.controller ? JSON.stringify(automationTask.controller, null, 2) : "no controller");
      return automationTask;
    } catch (error) {
      console.error(`\u274C Error fetching from Firestore for ${domainName}:`, error);
      return null;
    }
  }
  ...
  async getSyncStatus() {
    if (!this.indexedDBAvailable) {
      return {
        ...
        message: "IndexedDB not available - using direct Firestore access"
      };
    }
    try {
      const activeTasks = await dbManager.getActiveAutomationTasks();
      const allVersions = await dbManager.getAllVersions();
      return {
        ...this.syncStatus,
        ...
        indexedDBStats: {
          ...
          lastVersionSync: allVersions.length > 0 ? new Date(Math.max(...allVersions.map((v) => v.lastSync.getTime()))) : null
        }
      };
    } catch (error) {
      return {
  ...
  async verifyDataIntegrity() {
    if (!this.indexedDBAvailable) {
      return {
        indexedDBAvailable: false,
        message: "IndexedDB not available - cannot verify data integrity"
      };
    }
    try {
      console.log("\u{1F50D} Verifying data integrity...");
      const activeTasks = await dbManager.getActiveAutomationTasks();
      const allVersions = await dbManager.getAllVersions();
      const verification = {
        indexedDBAvailable: true,
        totalTasks: activeTasks.length,
        totalVersions: allVersions.length,
        tasksWithSteps: activeTasks.filter((t) => t.steps && t.steps.length > 0).length,
        tasksWithoutSteps: activeTasks.filter((t) => !t.steps || t.steps.length === 0).length,
        domains: Array.from(new Set(activeTasks.map((t) => t.domainName))),
        averageStepsPerTask: activeTasks.length > 0 ? activeTasks.reduce((sum, t) => sum + (t.steps?.length || 0), 0) / activeTasks.length : 0,
        lastModified: activeTasks.length > 0 ? new Date(Math.max(...activeTasks.map((t) => t.lastModified.getTime()))) : null,
        dataHealth: {
          hasData: activeTasks.length > 0,
          hasVersions: allVersions.length > 0,
          dataConsistency: activeTasks.length === allVersions.length,
          recentData: activeTasks.length > 0 ? Date.now() - Math.max(...activeTasks.map((t) => t.lastModified.getTime())) < 24 * 60 * 60 * 1e3 : false
        }
      };
  ...
  async getAutomationDetails(domainName) {
    try {
      const task = await this.getAutomationForDomain(domainName);
      if (!task) {
        return null;
      }
      return {
      ...
      console.error(`\u274C Error getting automation details for ${domainName}:`, error);
      return null;
    }
  }
  async getConsolidationSetupForDomain(domainName) {
    try {
      console.log(`\u{1F50D} Looking for consolidation setup for domain: ${domainName}`);
      if (!this.indexedDBAvailable) {
        console.log(`\u{1F4E1} IndexedDB not available - fetching consolidation setup directly from Firestore for ${domainName}`);
        return await this.getConsolidationSetupFromFirestore(domainName);
      }
      const setup = await dbManager.getConsolidationSetup(domainName);
      if (setup) {
        console.log(`\u2705 Found consolidation setup in IndexedDB for ${domainName}`);
        return setup;
      } else {
        console.log(`\u274C No consolidation setup found in IndexedDB for domain: ${domainName}`);
        console.log(`\u{1F4E1} Fetching consolidation setup from Firestore for ${domainName}`);
        return await this.getConsolidationSetupFromFirestore(domainName);
      }
    } catch (error) {
      console.error(`\u274C Error loading consolidation setup for ${domainName}:`, error);
      console.log(`\u{1F4E1} Attempting Firestore fallback for ${domainName}`);
      return await this.getConsolidationSetupFromFirestore(domainName);
    }
  }
  async getConsolidationSetupFromFirestore(domainName) {
    try {
      console.log(`\u{1F4E1} [CONSOLIDATION] Fetching consolidation setup directly from Firestore for: ${domainName}`);
      const authCheck = await new Promise((resolve) => {
        chrome.storage.local.get(["isLoggedIn", "userId"], (result) => {
          const isLoggedIn = result.isLoggedIn === true;
          const userId = result.userId || null;
      ...
      if (!authCheck.isAuthenticated) {
        console.error(`\u274C [CONSOLIDATION] User not authenticated - cannot fetch from Firestore`);
        return null;
      }
      console.log(`\u2705 [CONSOLIDATION] User authenticated: ${authCheck.userId}`);
      const normalizeDomain = (domain) => {
        if (!domain || typeof domain !== "string") return "";
        return domain.toLowerCase().trim().replace(/^www\./, "");
      };
      const normalizedDomain = normalizeDomain(domainName);
      console.log(`\u{1F50D} [CONSOLIDATION] Normalized domain: "${domainName}" -> "${normalizedDomain}"`);
      if (!normalizedDomain) {
        console.error(`\u274C [CONSOLIDATION] Invalid domain name: ${domainName}`);
        return null;
      }
      const consolidationSetupRef = collection(db, "consolidationSetup");
      console.log(`\u{1F50D} [CONSOLIDATION] Querying Firestore collection: consolidationSetup`);
      console.log(`\u{1F50D} [CONSOLIDATION] Attempting query: site == "${normalizedDomain}"`);
      let q2 = query(consolidationSetupRef, where("site", "==", normalizedDomain));
      let querySnapshot = await getDocs(q2);
      console.log(`\u{1F50D} [CONSOLIDATION] Query result (site): ${querySnapshot.size} documents found`);
      if (querySnapshot.empty && domainName !== normalizedDomain) {
        const originalDomainLower = domainName.toLowerCase().trim();
        console.log(`\u{1F50D} [CONSOLIDATION] No match with normalized domain, trying original: "${originalDomainLower}"`);
        q2 = query(consolidationSetupRef, where("site", "==", originalDomainLower));
        querySnapshot = await getDocs(q2);
        console.log(`\u{1F50D} [CONSOLIDATION] Query result (original): ${querySnapshot.size} documents found`);
      }
      if (querySnapshot.empty) {
        console.log(`\u{1F50D} [CONSOLIDATION] No match found with 'site' field, trying 'domainName' field...`);
        q2 = query(consolidationSetupRef, where("domainName", "==", normalizedDomain));
        querySnapshot = await getDocs(q2);
        console.log(`\u{1F50D} [CONSOLIDATION] Query result (domainName): ${querySnapshot.size} documents found`);
      }
      if (querySnapshot.empty) {
        console.log(`\u{1F50D} [CONSOLIDATION] No exact match found, fetching all consolidation setups for normalized comparison...`);
        try {
          const allDocs = await getDocs(consolidationSetupRef);
          console.log(`\u{1F50D} [CONSOLIDATION] Fetched ${allDocs.size} total consolidation setups from Firestore`);
          const allSites = allDocs.docs.map((doc3) => {
            const data = doc3.data();
            return {
              id: doc3.id,
              site: data.site,
          ...
          console.log(`\u{1F50D} [CONSOLIDATION] Available sites in Firestore:`, allSites);
          const matchingDoc = allDocs.docs.find((doc3) => {
            const data = doc3.data();
            const siteField = normalizeDomain(data.site || "");
            const domainNameField = normalizeDomain(data.domainName || "");
            const matches = siteField === normalizedDomain || domainNameField === normalizedDomain;
            if (matches) {
              console.log(`\u2705 [CONSOLIDATION] Found match: doc.id=${doc3.id}, site="${data.site}", domainName="${data.domainName}"`);
            }
            return matches;
          });
          if (matchingDoc) {
            console.log(`\u2705 [CONSOLIDATION] Found matching document via normalized comparison`);
            querySnapshot = {
              empty: false,
              docs: [matchingDoc],
              size: 1
            ...
            console.log(`\u274C [CONSOLIDATION] No matching document found after checking all ${allDocs.size} documents`);
            console.log(`\u{1F50D} [CONSOLIDATION] Searched for normalized domain: "${normalizedDomain}"`);
          }
        } catch (fetchAllError) {
          console.error(`\u274C [CONSOLIDATION] Error fetching all documents:`, fetchAllError);
        }
      }
      if (querySnapshot.empty) {
        console.log(`\u274C [CONSOLIDATION] No consolidation setup found in Firestore for domain: ${domainName} (normalized: ${normalizedDomain})`);
        console.log(`\u{1F4A1} [CONSOLIDATION] Tip: Make sure a document exists in 'consolidationSetup' collection with site="${normalizedDomain}" or domainName="${normalizedDomain}"`);
        return null;
      }
      const doc2 = querySnapshot.docs[0];
      const setupData = doc2.data();
      const domainField = setupData.site || setupData.domainName;
      if (!domainField || typeof domainField !== "string" || domainField.trim() === "") {
        console.warn(`\u26A0\uFE0F Consolidation setup ${doc2.id} has invalid or missing domain field`);
        return null;
      }
      let config = {};
      if (setupData.fields && Array.isArray(setupData.fields)) {
        console.log(`\u{1F4CA} Transforming fields array to config object (${setupData.fields.length} fields)`);
        setupData.fields.forEach((field) => {
          if (field && field.name && typeof field.name === "string") {
            config[field.name] = field.enabled === true ? "TRUE" : "FALSE";
          }
        });
        console.log(`\u2705 Transformed ${Object.keys(config).length} fields to config object`);
      } else if (setupData.config && typeof setupData.config === "object") {
        console.log(`\u{1F4CA} Using existing config object`);
        config = setupData.config;
      } else {
        console.warn(`\u26A0\uFE0F Consolidation setup ${doc2.id} has no fields array or config object`);
        config = {};
      }
      const consolidationSetup = {
        id: doc2.id,
        domainName: domainField.trim().toLowerCase(),
        // Normalize domain name
        config,
        lastModified: setupData.updatedAt?.toDate() || setupData.lastModified?.toDate() || /* @__PURE__ */ new Date(),
        createdAt: setupData.createdAt?.toDate() || /* @__PURE__ */ new Date()
      };
      console.log(`\u2705 Found consolidation setup in Firestore for ${domainName}`);
      console.log(`\u{1F4CA} Config keys: ${Object.keys(config).join(", ")}`);
      return consolidationSetup;
    } catch (error) {
      console.error(`\u274C Error fetching consolidation setup from Firestore for ${domainName}:`, error);
      return null;
    }
  }
  ...
  async syncToChromeStorage() {
    console.log("\u{1F504} Syncing automation data from IndexedDB to Chrome Storage...");
    try {
      if (!this.indexedDBAvailable) {
        console.log("\u26A0\uFE0F IndexedDB not available - falling back to Firestore for Chrome Storage sync");
        return await this.syncToChromeStorageFromFirestore();
      }
      console.log("\u{1F5D1}\uFE0F [CHROME_STORAGE] Clearing old automation_tasks from Chrome Storage...");
      await new Promise((resolve) => {
        chrome.storage.local.remove("automation_tasks", () => {
          if (chrome.runtime.lastError) {
            console.warn("\u26A0\uFE0F [CHROME_STORAGE] Could not clear old automation_tasks:", chrome.runtime.lastError);
          } else {
            console.log("\u2705 [CHROME_STORAGE] Cleared old automation_tasks from Chrome Storage");
          }
          resolve();
        });
      });
      ...
      console.log(`\u{1F4DA} Syncing from IndexedDB to Chrome Storage (user role: ${userRole})`);
      let allTasks = await dbManager.getAllAutomationTasks();
      console.log(`\u{1F4CA} Found ${allTasks.length} total tasks in IndexedDB`);
      let automationTasks;
      if (userRole === "admin") {
        console.log(`\u{1F4DA} [ADMIN] Including ALL automation tasks (no status filter)`);
        automationTasks = allTasks;
      } else {
        console.log(`\u{1F4DA} Filtering tasks by status: active, completed, or hold (for ${userRole} role)`);
        const statusesToFetch = ["active", "completed", "hold"];
        automationTasks = allTasks.filter((task) => statusesToFetch.includes(task.status));
        console.log(`\u{1F4CA} Filtered to ${automationTasks.length} tasks (from ${allTasks.length} total)`);
      }
      const tasksForStorage = automationTasks.map((task) => ({
        id: task.id,
        assignPerson: task.assignPerson,
        controller: task.controller || {},
        settings: {
          maxRetries: task.settings?.maxRetries || task.controller?.settings?.maxRetries || 3,
          notifyOnComplete: task.settings?.notifyOnComplete || task.controller?.settings?.notifyOnComplete || true,
          screenshotOnError: task.settings?.screenshotOnError || task.controller?.settings?.screenshotOnError || true,
          timeout: task.settings?.timeout || task.controller?.settings?.timeout || 3e4
        },
        steps: task.steps || task.controller?.steps || [],
      ...
      chrome.storage.local.set({
        automation_tasks: tasksForStorage,
        automation_sync_timestamp: Date.now(),
        automation_sync_count: tasksForStorage.length
      }, () => {
        if (chrome.runtime.lastError) {
          console.error("\u274C Error saving automation data to Chrome Storage:", chrome.runtime.lastError);
        } else {
          console.log(`\u{1F4BE} Synced ${tasksForStorage.length} automation tasks from IndexedDB to Chrome Storage`);
        }
      });
    } catch (error) {
      console.error("\u274C Error syncing from IndexedDB to Chrome Storage:", error);
      console.log("\u{1F504} Falling back to Firestore for Chrome Storage sync...");
      try {
        await this.syncToChromeStorageFromFirestore();
      } catch (fallbackError) {
        console.error("\u274C Error in Firestore fallback sync:", fallbackError);
      }
    }
  }
  // Fallback method: Sync from Firestore to Chrome Storage (used when IndexedDB is not available)
  async syncToChromeStorageFromFirestore() {
    console.log("\u{1F504} Syncing automation data from Firestore to Chrome Storage (fallback)...");
    try {
      const collectionName = await getAutomationCollectionName();
      console.log(`\u{1F4DA} Syncing to Chrome Storage from collection: ${collectionName}`);
      console.log(`\u{1F3AD} Role-based collection selection: ${collectionName} (user role determines this)`);
      const userRole = await new Promise((resolve) => {
        chrome.storage.local.get(["user_roles"], (result) => {
          resolve(result.user_roles?.role || "user");
      ...
      let querySnapshot;
      if (userRole === "admin") {
        console.log(`\u{1F4DA} [ADMIN] Fetching ALL automation tasks for Chrome Storage (no status filter)`);
        const q2 = query(automationTasksRef);
        querySnapshot = await getDocs(q2);
      } else {
        console.log(`\u{1F4DA} Fetching automation tasks for Chrome Storage (filtered by status)`);
        const statusesToFetch = ["active", "completed", "hold"];
        const q2 = query(automationTasksRef, where("status", "in", statusesToFetch));
        querySnapshot = await getDocs(q2);
      }
      const automationTasks = querySnapshot.docs.map((doc2) => {
        const data = doc2.data();
        return {
          id: doc2.id,
          ...
          settings: {
            maxRetries: data.settings?.maxRetries || data.controller?.settings?.maxRetries || 3,
            notifyOnComplete: data.settings?.notifyOnComplete || data.controller?.settings?.notifyOnComplete || true,
            screenshotOnError: data.settings?.screenshotOnError || data.controller?.settings?.screenshotOnError || true,
            timeout: data.settings?.timeout || data.controller?.settings?.timeout || 3e4
          },
          steps: data.steps || data.controller?.steps || [],
          createdAt: data.createdAt?.toDate() || /* @__PURE__ */ new Date(),
          createdBy: data.createdBy,
          domainName: data.domainName,
          lastModified: data.lastModified?.toDate() || /* @__PURE__ */ new Date(),
      ...
      chrome.storage.local.set({
        ...
        automation_sync_timestamp: Date.now(),
        automation_sync_count: automationTasks.length
      }, () => {
        if (chrome.runtime.lastError) {
          console.error("\u274C Error saving automation data to Chrome Storage:", chrome.runtime.lastError);
        } else {
          console.log(`\u{1F4BE} Synced ${automationTasks.length} automation tasks from Firestore to Chrome Storage`);
        }
      });
    } catch (error) {
      console.error("\u274C Error syncing from Firestore to Chrome Storage:", error);
      throw error;
    }
  }
};
var automationSync = new AutomationSyncService();
async function getAutomationCollectionName() {
  return new Promise((resolve) => {
    chrome.storage.local.get(["user_roles"], (result) => {
      const userRole = result.user_roles?.role || "user";
      console.log(`\u{1F3AD} User role: ${userRole}`);
      if (userRole === "collaborator" || userRole === "admin") {
        console.log(`\u{1F4DA} Using automationTasks collection for ${userRole} role (testing access)`);
        resolve("automationTasks");
      } else if (userRole === "user") {
        console.log(`\u{1F4DA} Using automationSteps collection for user role`);
        resolve("automationSteps");
      } else {
        console.log(`\u26A0\uFE0F Unknown role "${userRole}", defaulting to automationSteps`);
        resolve("automationSteps");
      }
    });
  });
}
async function incrementCitationAccumulation(userId) {
  try {
    console.log("\u{1F4CA} Incrementing citation accumulation for user:", userId);
    const currentDate = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    console.log("\u{1F4C5} Current date:", currentDate);
    const citationAccumulationRef = doc(db, "citation_accumulation", `${userId}_${currentDate}`);
    const docSnap = await getDoc(citationAccumulationRef);
    if (docSnap.exists()) {
      console.log("\u{1F504} Updating existing citation accumulation document");
      await updateDoc(citationAccumulationRef, {
        citationAccumulated: increment(1),
        lastUpdated: /* @__PURE__ */ new Date()
      });
      console.log("\u2705 Citation accumulation updated successfully");
    } else {
      console.log("\u{1F195} Creating new citation accumulation document");
      await setDoc(citationAccumulationRef, {
        userId,
        createdDate: currentDate,
        citationAccumulated: 1,
        createdAt: /* @__PURE__ */ new Date(),
        lastUpdated: /* @__PURE__ */ new Date()
      });
      console.log("\u2705 New citation accumulation document created successfully");
    }
    return { success: true };
  } catch (error) {
    console.error("\u274C Error incrementing citation accumulation:", error);
  ...
  try {
    ...
    if (enabled) {
      ...
      for (const size of iconSizes) {
        const iconPath = size === 128 ? "logo512.png" : "logo192.png";
      ...
      chrome.action.setIcon({
        ...
        if (chrome.runtime.lastError) {
          console.error("\u274C [ICON] Error setting colored icon:", chrome.runtime.lastError);
        } else {
          console.log("\u2705 [ICON] Extension icon set to colored (enabled)");
        }
      });
    } else {
      const iconSizes = [16, 32, 48, 128];
      ...
      chrome.action.setIcon({
        ...
        if (chrome.runtime.lastError) {
          console.error("\u274C [ICON] Error setting grayscale icon:", chrome.runtime.lastError);
        } else {
          console.log("\u2705 [ICON] Extension icon set to grayscale (disabled)");
        }
      });
    }
  } catch (error) {
    console.error("\u274C [ICON] Error updating extension icon:", error);
    chrome.action.setIcon({
      path: {
        "16": "logo192.png",
        "32": "logo192.png",
...
chrome.storage.local.get(["autoStartEnabled"], (result) => {
  const autoStartEnabled = result.autoStartEnabled !== false;
  updateExtensionIcon(autoStartEnabled).catch((error) => {
    console.error("\u274C [ICON] Error initializing extension icon:", error);
  });
});
  ...
  try {
    chrome.storage.local.get(["notificationsEnabled"], (result) => {
      ...
      if (!notificationsEnabled) {
        console.log("\u{1F515} [NOTIFICATION] Notifications are disabled by user preference");
        return;
      }
      showNotificationInternal(title, message);
    });
  } catch (error) {
    console.error("\u274C [NOTIFICATION] Unexpected error in showNotification:", error);
    if (error instanceof Error) {
      console.error("\u274C [NOTIFICATION] Error stack:", error.stack);
    }
  }
}
function showNotificationInternal(title, message) {
  try {
    if (!chrome || !chrome.notifications) {
      console.warn("\u26A0\uFE0F [NOTIFICATION] Chrome notifications API not available.");
      return;
    }
    const notificationOptions = {
      type: "basic",
      iconUrl: chrome.runtime.getURL("logo192.png"),
      title: title || "CB-PHAA Notification",
    ...
    chrome.notifications.getPermissionLevel((level) => {
      if (chrome.runtime.lastError) {
        console.error("\u274C [NOTIFICATION] Error checking permission:", chrome.runtime.lastError.message);
      } else if (level !== "granted") {
        console.warn(`\u26A0\uFE0F [NOTIFICATION] Permission level: ${level} (may still work if declared in manifest)`);
      } else {
        console.log("\u2705 [NOTIFICATION] Permission granted");
      }
      chrome.notifications.create(notificationOptions, (notificationId) => {
        if (chrome.runtime.lastError) {
          console.error("\u274C [NOTIFICATION] Failed to create notification:", chrome.runtime.lastError.message);
        ...
        if (notificationId) {
          console.log("\u2705 [NOTIFICATION] Notification created successfully:", notificationId, {
            title: notificationOptions.title,
            message: notificationOptions.message
          });
          console.log("\u{1F4CC} [NOTIFICATION] Notification will persist in notification panel until user dismisses it");
        } else {
          console.warn("\u26A0\uFE0F [NOTIFICATION] Notification created but no ID returned");
        }
      });
    });
  } catch (error) {
    console.error("\u274C [NOTIFICATION] Unexpected error in showNotificationInternal:", error);
    if (error instanceof Error) {
      console.error("\u274C [NOTIFICATION] Error stack:", error.stack);
    }
  }
}
function listenToUserPlanSubscriber(userId) {
  if (!auth.currentUser) {
    console.warn("\u26A0\uFE0F User not authenticated, skipping plan_subscribers listener setup");
    return;
  }
  if (auth.currentUser.uid !== userId) {
    console.warn("\u26A0\uFE0F User ID mismatch for plan_subscribers. Auth user:", auth.currentUser.uid, "Requested user:", userId);
    return;
  }
  auth.currentUser.getIdToken(true).then((idToken) => {
    if (!idToken) {
      console.error("\u274C No valid ID token found for plan_subscribers");
      return;
    }
    console.log("\u2705 User has valid ID token for plan_subscribers, proceeding with listener setup");
  }).catch((error) => {
    console.error("\u274C Error getting ID token for plan_subscribers:", error);
    return;
  });
  if (unsubscribePlanSubscriber) {
    unsubscribePlanSubscriber();
  }
  const planRef = doc(db, "plan_subscribers", userId);
  console.log("\u{1F4C4} Listening to plan_subscribers/" + userId);
  unsubscribePlanSubscriber = onSnapshot(planRef, (docSnap) => {
    if (docSnap.exists()) {
      const data = docSnap.data();
      console.log("\u2705 plan_subscribers exists:", data);
      chrome.storage.local.set({ plan_subscribers: data }, () => {
        if (chrome.runtime.lastError) {
          console.error("\u274C Error saving plan_subscribers to storage:", chrome.runtime.lastError);
        } else {
          console.log("\u{1F4BE} plan_subscribers saved to chrome.storage.local successfully");
        }
      });
    } else {
      chrome.storage.local.remove("plan_subscribers", () => {
        if (chrome.runtime.lastError) {
          console.error("\u274C Error removing plan_subscribers from storage:", chrome.runtime.lastError);
        } else {
          console.log("\u{1F5D1}\uFE0F plan_subscribers removed from chrome.storage.local");
        }
      });
    }
  }, (error) => {
    console.error("\u274C Firestore listener error (plan_subscribers):", error);
    if (error.code === "permission-denied") {
      console.error("\u{1F6AB} Permission denied - user may not have access to plan_subscribers collection");
      showNotification("CB-PHAA Permission Error", "You do not have permission to access plan data. Please contact support.");
    } else if (error.code === "unauthenticated") {
      console.error("\u{1F510} User not authenticated for plan_subscribers");
      showNotification("CB-PHAA Auth Error", "Please log in again to access your plan data.");
    } else if (error.code === "unavailable") {
      console.error("\u{1F310} Firestore service unavailable for plan_subscribers");
      showNotification("CB-PHAA Service Error", "Plan service is temporarily unavailable. Will retry automatically.");
    } else {
      console.error("\u274C Unknown Firestore error for plan_subscribers:", error.code, error.message);
      showNotification("CB-PHAA Error", `Failed to sync plan data: ${error.message}`);
    }
  });
}
function listenToUserRole(userId) {
  if (!auth.currentUser) {
    console.warn("\u26A0\uFE0F User not authenticated, skipping users listener setup");
    return;
  }
  if (auth.currentUser.uid !== userId) {
    console.warn("\u26A0\uFE0F User ID mismatch for users. Auth user:", auth.currentUser.uid, "Requested user:", userId);
    return;
  }
  auth.currentUser.getIdToken(true).then((idToken) => {
    if (!idToken) {
      console.error("\u274C No valid ID token found for users");
      return;
    }
    console.log("\u2705 User has valid ID token for users, proceeding with listener setup");
  }).catch((error) => {
    console.error("\u274C Error getting ID token for users:", error);
    return;
  });
  if (unsubscribeUserRole) {
    unsubscribeUserRole();
  ...
  console.log("\u{1F4C4} Listening to users/" + userId);
  unsubscribeUserRole = onSnapshot(userRef, (docSnap) => {
    if (docSnap.exists()) {
      const data = docSnap.data();
      console.log("\u2705 users document exists:", data);
      const userRole = {
        role: data.role || "user",
        ...
        email: data.email,
        displayName: data.displayName,
        lastUpdated: data.lastUpdated || /* @__PURE__ */ new Date()
      };
      chrome.storage.local.set({ user_roles: userRole }, () => {
        if (chrome.runtime.lastError) {
          console.error("\u274C Error saving user_roles to storage:", chrome.runtime.lastError);
        } else {
          console.log("\u{1F4BE} user_roles saved to chrome.storage.local successfully");
        }
      });
    } else {
      console.log("\u26A0\uFE0F users document does not exist for user:", userId);
      chrome.storage.local.remove("user_roles", () => {
        if (chrome.runtime.lastError) {
          console.error("\u274C Error removing user_roles from storage:", chrome.runtime.lastError);
        } else {
          console.log("\u{1F5D1}\uFE0F user_roles removed from chrome.storage.local");
        }
      });
    }
  }, (error) => {
    ...
    if (error.code === "permission-denied") {
      console.error("\u{1F6AB} Permission denied - user may not have access to users collection");
      showNotification("CB-PHAA Permission Error", "You do not have permission to access user data. Please contact support.");
    } else if (error.code === "unauthenticated") {
      console.error("\u{1F510} User not authenticated for users collection");
      showNotification("CB-PHAA Auth Error", "Please log in again to access your user data.");
    } else if (error.code === "unavailable") {
      console.error("\u{1F310} Firestore service unavailable for users collection");
      showNotification("CB-PHAA Service Error", "User service is temporarily unavailable. Will retry automatically.");
    } else {
      console.error("\u274C Unknown Firestore error for users:", error.code, error.message);
      showNotification("CB-PHAA Error", `Failed to sync user data: ${error.message}`);
    }
  });
}
function setupAuthListener() {
  onAuthStateChanged(auth, (user) => {
    console.log("\u{1F510} Auth state changed. User:", user ? user.uid : "null");
    if (user) {
      console.log("\u{1F464} User authenticated:", user.uid);
      let tokenAttempts = 0;
      const maxTokenAttempts = 3;
      const attemptTokenRefresh = () => {
        tokenAttempts++;
        console.log(`\u{1F504} Token refresh attempt ${tokenAttempts}/${maxTokenAttempts}`);
        user.getIdToken(true).then(async (idToken) => {
          if (!idToken) {
            console.error(`\u274C Failed to get ID token for authenticated user (attempt ${tokenAttempts})`);
            if (tokenAttempts < maxTokenAttempts) {
              console.log(`\u23F3 Retrying token refresh in 2 seconds...`);
              setTimeout(attemptTokenRefresh, 2e3);
            } else {
              chrome.storage.local.set({ CampaignDataStatus: "token_failed" });
            }
            return;
          }
          ...
          chrome.storage.local.set({
            isLoggedIn: true,
            userId: user.uid,
            CampaignDataStatus: "authenticated"
          });
          setTimeout(async () => {
            console.log("\u{1F504} Setting up Firestore listeners after auth delay...");
            if (auth.currentUser && auth.currentUser.uid === user.uid) {
              console.log("\u2705 User still authenticated, testing Firestore access before setting up listeners");
              let accessTestAttempts = 0;
              const maxAccessTestAttempts = 3;
              const testFirestoreAccessWithRetry = async () => {
                accessTestAttempts++;
                console.log(`\u{1F50D} Firestore access test attempt ${accessTestAttempts}/${maxAccessTestAttempts}`);
                const testResult = await testFirestoreAccess();
                if (!testResult.success) {
                  console.error(`\u274C Firestore access test failed (attempt ${accessTestAttempts}):`, testResult.error);
                  if (accessTestAttempts < maxAccessTestAttempts) {
                    console.log(`\u23F3 Retrying Firestore access test in 3 seconds...`);
                    setTimeout(testFirestoreAccessWithRetry, 3e3);
                  } else {
                    console.error("\u274C All Firestore access tests failed, cannot set up listeners");
                    chrome.storage.local.set({ CampaignDataStatus: "firestore_access_failed" });
                    showNotification("CB-PHAA Firestore Error", `Cannot access Firestore after ${maxAccessTestAttempts} attempts: ${testResult.error}`);
                  }
                  return;
                }
                console.log("\u2705 Firestore access test passed, setting up listeners");
                try {
                  console.log("\u{1F504} Setting up CampaignData listener...");
                  listenToUserCampaignData(user.uid);
                  setTimeout(() => {
                    console.log("\u{1F504} Setting up plan_subscribers listener...");
                    listenToUserPlanSubscriber(user.uid);
                  }, 1e3);
                  setTimeout(() => {
                    console.log("\u{1F504} Setting up users listener...");
                  ...
                  chrome.storage.local.set({ CampaignDataStatus: "listener_setup_error" });
                }
              };
              testFirestoreAccessWithRetry();
            } else {
              console.warn("\u26A0\uFE0F User authentication changed during delay, skipping listener setup");
              chrome.storage.local.set({ CampaignDataStatus: "auth_changed" });
            }
          }, 3e3);
        }).catch((error) => {
          console.error(`\u274C Error getting ID token (attempt ${tokenAttempts}):`, error);
          if (tokenAttempts < maxTokenAttempts) {
            console.log(`\u23F3 Retrying token refresh in 2 seconds...`);
            setTimeout(attemptTokenRefresh, 2e3);
          } else {
            chrome.storage.local.set({ CampaignDataStatus: "token_error" });
          }
        });
      };
      attemptTokenRefresh();
    } else {
      console.log("\u{1F6AA} User signed out");
      if (unsubscribeCampaignData) {
        console.log("\u{1F504} Unsubscribing from Firestore listener");
        unsubscribeCampaignData();
      }
      if (unsubscribePlanSubscriber) {
        console.log("\u{1F504} Unsubscribing from plan_subscribers listener");
        unsubscribePlanSubscriber();
      }
      if (unsubscribeUserRole) {
        console.log("\u{1F504} Unsubscribing from user_roles listener");
        unsubscribeUserRole();
      }
      chrome.storage.local.remove(["CampaignData", "plan_subscribers", "isLoggedIn", "CampaignDataStatus", "user_roles"], () => {
        if (chrome.runtime.lastError) {
          console.error("\u274C Error clearing storage on logout:", chrome.runtime.lastError);
        } else {
          console.log("\u{1F9F9} Storage cleared on logout");
        }
      });
    }
  });
}
function listenToUserCampaignData(userId) {
  console.log("\u{1F442} Setting up Firestore listener for user:", userId);
  if (!auth.currentUser) {
    console.warn("\u26A0\uFE0F User not authenticated, skipping CampaignData listener setup");
    chrome.storage.local.set({ CampaignDataStatus: "not_authenticated" });
    return;
  }
  if (auth.currentUser.uid !== userId) {
    console.warn("\u26A0\uFE0F User ID mismatch. Auth user:", auth.currentUser.uid, "Requested user:", userId);
    chrome.storage.local.set({ CampaignDataStatus: "user_mismatch" });
    return;
  }
  auth.currentUser.getIdToken(true).then((idToken) => {
    if (!idToken) {
      console.error("\u274C No valid ID token found for user");
      chrome.storage.local.set({ CampaignDataStatus: "no_token" });
      return;
    }
    console.log("\u2705 User has valid ID token, proceeding with listener setup");
    console.log("\u{1F50D} Token details:", {
      userId: auth.currentUser.uid,
  ...
  if (unsubscribeCampaignData) {
    console.log("\u{1F504} Unsubscribing from previous listener");
    unsubscribeCampaignData();
  }
  const docRef = doc(db, "CampaignData", userId);
  console.log("\u{1F4C4} Listening to document:", `CampaignData/${userId}`);
  unsubscribeCampaignData = onSnapshot(
    docRef,
    (docSnap) => {
      console.log("\u{1F4CA} Firestore document update received");
      if (docSnap.exists()) {
        const data = docSnap.data();
        console.log("\u2705 CampaignData exists:", data);
        chrome.storage.local.set({ CampaignData: data, CampaignDataStatus: "ok" }, () => {
          if (chrome.runtime.lastError) {
            console.error("\u274C Error saving CampaignData to storage:", chrome.runtime.lastError);
            chrome.storage.local.set({ CampaignDataStatus: "error" });
            showNotification("CB-PHAA Error", "Failed to save CampaignData to storage.");
          } else {
            console.log("\u{1F4BE} CampaignData saved to chrome.storage.local successfully");
          }
        });
      } else {
        console.log("\u26A0\uFE0F CampaignData document does not exist for user:", userId);
        chrome.storage.local.set({ CampaignDataStatus: "missing" });
        chrome.storage.local.remove("CampaignData", () => {
          if (chrome.runtime.lastError) {
            console.error("\u274C Error removing CampaignData from storage:", chrome.runtime.lastError);
            chrome.storage.local.set({ CampaignDataStatus: "error" });
            showNotification("CB-PHAA Error", "Failed to remove CampaignData from storage.");
          } else {
            showNotification("CB-PHAA Warning", "No CampaignData found for your account.");
            console.log("\u{1F5D1}\uFE0F CampaignData removed from chrome.storage.local");
          }
        });
      }
    },
    (error) => {
      console.error("\u274C Firestore listener error (CampaignData):", error);
      if (error.code === "permission-denied") {
        console.error("\u{1F6AB} Permission denied - user may not have access to CampaignData collection");
        chrome.storage.local.set({ CampaignDataStatus: "permission_denied" });
        console.log("\u{1F504} Attempting authentication recovery for permission error...");
        handleAuthRecovery();
        showNotification("CB-PHAA Permission Error", "Permission denied. Attempting to recover authentication...");
        setTimeout(() => {
          if (auth.currentUser && auth.currentUser.uid === userId) {
            console.log("\u{1F504} Retrying CampaignData listener after permission recovery...");
            retryListener("CampaignData", userId, () => listenToUserCampaignData(userId));
          }
        }, 3e3);
      } else if (error.code === "unauthenticated") {
        console.error("\u{1F510} User not authenticated - redirecting to login");
        chrome.storage.local.set({ CampaignDataStatus: "unauthenticated" });
        showNotification("CB-PHAA Auth Error", "Please log in again to access your data.");
      } else if (error.code === "unavailable") {
        console.error("\u{1F310} Firestore service unavailable - will retry later");
        chrome.storage.local.set({ CampaignDataStatus: "service_unavailable" });
        showNotification("CB-PHAA Service Error", "Firestore service is temporarily unavailable. Will retry automatically.");
        retryListener("CampaignData", userId, () => listenToUserCampaignData(userId));
      } else {
        console.error("\u274C Unknown Firestore error:", error.code, error.message);
        chrome.storage.local.set({ CampaignDataStatus: "error" });
        showNotification("CB-PHAA Error", `Failed to sync CampaignData: ${error.message}`);
        retryListener("CampaignData", userId, () => listenToUserCampaignData(userId));
      }
    }
  );
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message && message.action === "updateAutomationStatus") {
    ...
    chrome.storage.local.set({
      ...
      lastAutomationUpdate: (/* @__PURE__ */ new Date()).toISOString()
    }, () => {
      if (chrome.runtime.lastError) {
        console.error("\u274C Error storing automation status:", chrome.runtime.lastError);
        sendResponse({ success: false, error: chrome.runtime.lastError.message });
      } else {
        console.log("\u2705 Automation status stored successfully");
        sendResponse({ success: true });
      }
    });
    return true;
  }
  if (message && message.type === "FETCH_CAMPAIGN_DATA") {
    console.log("\u{1F4CB} Fetching CampaignData from chrome.storage.local (not IndexedDB)");
    chrome.storage.local.get(["CampaignData", "CampaignDataStatus", "isLoggedIn", "userId"], (result) => {
      if (chrome.runtime.lastError) {
        console.error("\u274C Error fetching CampaignData:", chrome.runtime.lastError);
        sendResponse({ error: chrome.runtime.lastError.message });
      } else {
        console.log("\u{1F4E4} Sending CampaignData to popup:", result.CampaignData);
        if (!result.CampaignData && result.isLoggedIn && result.userId) {
          console.log("\u{1F4CB} No CampaignData in storage, attempting direct Firestore fetch...");
          const campaignRef = doc(db, "CampaignData", result.userId);
          getDoc(campaignRef).then((docSnap) => {
            if (docSnap.exists()) {
              const data = docSnap.data();
              console.log("\u2705 CampaignData fetched directly from Firestore:", data);
              chrome.storage.local.set({ CampaignData: data }, () => {
                sendResponse({ CampaignData: data, CampaignDataStatus: "fetched_from_firestore" });
              });
            } else {
              console.log("\u26A0\uFE0F No CampaignData document found in Firestore");
              sendResponse({ CampaignData: null, CampaignDataStatus: "not_found" });
            }
          }).catch((error) => {
            console.error("\u274C Error fetching CampaignData from Firestore:", error);
            sendResponse({ CampaignData: result.CampaignData, CampaignDataStatus: result.CampaignDataStatus, error: error.message });
          });
        } else {
          sendResponse({ CampaignData: result.CampaignData, CampaignDataStatus: result.CampaignDataStatus });
        }
      }
    });
    return true;
  }
  if (message && message.type === "REFRESH_CAMPAIGN_DATA") {
    console.log("\u{1F504} Force refreshing CampaignData from Firestore...");
    ...
    const safeSendResponse = (response) => {
      if (!responseSent) {
        ...
        try {
          ...
          console.error("\u274C [REFRESH] Error sending response:", error);
        }
      }
    };
    chrome.storage.local.get(["isLoggedIn", "userId"], (result) => {
      ...
      if (!result.isLoggedIn || !result.userId) {
        console.error("\u274C User not logged in, cannot refresh CampaignData");
        safeSendResponse({ success: false, error: "User not logged in" });
        return;
      }
      if (!auth.currentUser || auth.currentUser.uid !== result.userId) {
        console.error("\u274C User authentication mismatch");
        safeSendResponse({ success: false, error: "User authentication mismatch" });
        return;
      }
      console.log(`\u{1F5D1}\uFE0F [REFRESH] Clearing CampaignData from chrome.storage.local...`);
      chrome.storage.local.remove("CampaignData", () => {
        if (chrome.runtime.lastError) {
          console.error("\u274C [REFRESH] Error clearing CampaignData from storage:", chrome.runtime.lastError);
        } else {
          console.log("\u2705 [REFRESH] CampaignData cleared from chrome.storage.local");
        }
        const campaignRef = doc(db, "CampaignData", result.userId);
        console.log(`\u{1F50D} [REFRESH] Fetching CampaignData from Firestore for user: ${result.userId}`);
        getDoc(campaignRef).then((docSnap) => {
          if (docSnap.exists()) {
            const data = docSnap.data();
            console.log("\u2705 [REFRESH] CampaignData refreshed from Firestore");
            console.log("\u{1F4CA} [REFRESH] Data keys:", Object.keys(data));
            console.log("\u{1F4CA} [REFRESH] Citations count:", data.citations ? Array.isArray(data.citations) ? data.citations.length : "not array" : "no citations");
            chrome.storage.local.set({ CampaignData: data, CampaignDataStatus: "ok" }, () => {
              if (chrome.runtime.lastError) {
                console.error("\u274C [REFRESH] Error saving CampaignData to storage:", chrome.runtime.lastError);
                safeSendResponse({ success: false, error: chrome.runtime.lastError.message });
                return;
              }
              chrome.storage.local.get(["CampaignData"], (verifyResult) => {
                if (chrome.runtime.lastError) {
                  console.error("\u274C [REFRESH] Error verifying CampaignData write:", chrome.runtime.lastError);
                  safeSendResponse({ success: false, error: "Storage write verification failed" });
                  return;
                }
                const storedCitations = verifyResult.CampaignData?.citations;
                const storedCount = storedCitations ? Array.isArray(storedCitations) ? storedCitations.length : "not array" : "no citations";
                console.log("\u2705 [REFRESH] CampaignData write verified - stored citations count:", storedCount);
                console.log("\u{1F4BE} [REFRESH] CampaignData updated in chrome.storage.local successfully");
                console.log("\u{1F504} [REFRESH] Syncing IndexedDB and Chrome Storage...");
                automationSync.syncAllAutomations().then(() => {
                  console.log("\u2705 [REFRESH] IndexedDB sync completed");
                  return automationSync.syncToChromeStorage();
                }).then(() => {
                  console.log("\u2705 [REFRESH] Chrome Storage sync completed");
                  console.log("\u2705 [REFRESH] All syncs completed (IndexedDB + Chrome Storage)");
                  safeSendResponse({ success: true, CampaignData: data, CampaignDataStatus: "ok" });
                }).catch((syncError) => {
                  console.error("\u274C [REFRESH] Sync failed:", syncError);
                  safeSendResponse({ success: true, CampaignData: data, CampaignDataStatus: "ok", syncError: syncError.message });
                });
              });
            });
          } else {
            console.log("\u26A0\uFE0F [REFRESH] No CampaignData document found in Firestore");
            chrome.storage.local.set({ CampaignDataStatus: "missing" });
            chrome.storage.local.remove("CampaignData", () => {
              if (chrome.runtime.lastError) {
                console.error("\u274C [REFRESH] Error removing CampaignData from storage:", chrome.runtime.lastError);
                safeSendResponse({ success: false, error: chrome.runtime.lastError.message });
              } else {
                console.log("\u{1F5D1}\uFE0F [REFRESH] CampaignData removed from chrome.storage.local");
                console.log("\u{1F504} [REFRESH] Syncing IndexedDB and Chrome Storage...");
                automationSync.syncAllAutomations().then(() => {
                  console.log("\u2705 [REFRESH] IndexedDB sync completed");
                  return automationSync.syncToChromeStorage();
                  ...
                  console.log("\u2705 [REFRESH] Chrome Storage sync completed");
                  console.log("\u2705 [REFRESH] All syncs completed (IndexedDB + Chrome Storage)");
                  safeSendResponse({ success: true, CampaignData: null, CampaignDataStatus: "missing" });
                }).catch((syncError) => {
                  console.error("\u274C [REFRESH] Sync failed:", syncError);
                  safeSendResponse({ success: true, CampaignData: null, CampaignDataStatus: "missing", syncError: syncError.message });
                });
              }
            });
          }
        }).catch((error) => {
          console.error("\u274C [REFRESH] Error fetching CampaignData from Firestore:", error);
          ...
          chrome.storage.local.set({ CampaignDataStatus: "error" });
          console.log("\u{1F504} [REFRESH] Attempting IndexedDB and Chrome Storage sync despite CampaignData fetch error...");
          automationSync.syncAllAutomations().then(() => {
            console.log("\u2705 [REFRESH] IndexedDB sync completed (despite CampaignData error)");
            return automationSync.syncToChromeStorage();
          }).then(() => {
            console.log("\u2705 [REFRESH] Chrome Storage sync completed (despite CampaignData error)");
            console.log("\u2705 [REFRESH] All syncs completed (IndexedDB + Chrome Storage)");
            safeSendResponse({ success: false, error: error.message, indexedDBSynced: true });
          }).catch((syncError) => {
            console.error("\u274C [REFRESH] Sync also failed:", syncError);
            safeSendResponse({ success: false, error: error.message, indexedDBSynced: false, syncError: syncError.message });
          });
        });
      });
    });
  ...
  if (message && message.type === "FETCH_USER_ROLES") {
    console.log("\u{1F4CB} Fetching user roles from storage");
    chrome.storage.local.get(["user_roles"], (result) => {
      if (chrome.runtime.lastError) {
        console.error("\u274C Error fetching user roles:", chrome.runtime.lastError);
        sendResponse({ error: chrome.runtime.lastError.message });
      } else {
        console.log("\u{1F4E4} Sending user roles to popup:", result.user_roles);
        sendResponse({ user_roles: result.user_roles });
      }
    });
    return true;
  }
  if (message && message.type === "SHOW_AUTOMATION_NOTIFICATION") {
    const domain = message.domain || "Unknown Domain";
    const action = message.action || "unknown";
    const originalTitle = message.title || "CB-PHAA Automation";
    const originalMessage = message.message || "Automation status update";
    let actionEmoji = "\u{1F514}";
    let actionText = "";
    if (action === "error") {
      actionEmoji = "\u274C";
      actionText = "Error";
    ...
    const formattedTitle = `${actionEmoji} CB-PHAA Automation - ${domain}`;
    const formattedMessage = `[${actionText}] ${originalMessage}`;
    console.log("\u{1F514} [NOTIFICATION] Showing notification:", formattedTitle, formattedMessage, `[Domain: ${domain}, Action: ${action}]`);
    showNotification(formattedTitle, formattedMessage);
    sendResponse({ success: true });
    return true;
  }
  if (message && message.type === "CHECK_DOMAIN_AUTOMATION") {
    const domain = message.domain;
    if (!domain) {
      sendResponse({ hasAutomation: false });
      return true;
    }
    automationSync.getAutomationForDomain(domain).then((automation) => {
      const hasAutomation = !!automation;
      sendResponse({ hasAutomation });
      if (!hasAutomation) {
        console.log(`\u{1F6AB} [PERF] No automation for ${domain}, content script can exit early`);
      }
    }).catch(() => {
      sendResponse({ hasAutomation: true });
    });
    return true;
  }
  if (message && message.type === "UPDATE_NOTIFICATION_PREFERENCE") {
    const enabled = message.enabled !== false;
    console.log("\u{1F514} [NOTIFICATION] Updating notification preference:", enabled);
    chrome.storage.local.set({ notificationsEnabled: enabled }, () => {
      if (chrome.runtime.lastError) {
        console.error("\u274C [NOTIFICATION] Error saving preference:", chrome.runtime.lastError);
        sendResponse({ success: false, error: chrome.runtime.lastError.message });
      } else {
        console.log("\u2705 [NOTIFICATION] Notification preference saved:", enabled);
        sendResponse({ success: true });
      }
    });
    return true;
  }
  if (message && message.type === "UPDATE_AUTOSTART_PREFERENCE") {
    const enabled = message.enabled !== false;
    console.log("\u{1F3AF} [AUTOSTART] Updating auto-start preference:", enabled);
    updateExtensionIcon(enabled).then(() => {
      sendResponse({ success: true });
    }).catch((error) => {
      console.error("\u274C [AUTOSTART] Error updating extension icon:", error);
      sendResponse({ success: false, error: error.message });
    });
    return true;
  }
  if (message && message.type === "INCREMENT_CITATION") {
    const planDocId = message.planDocId;
    if (!planDocId) {
      sendResponse({ success: false, error: "No planDocId provided" });
      return;
    }
    const planRef = doc(db, "plan_subscribers", planDocId);
    updateDoc(planRef, { "plans.usedCitation": increment(1) }).then(() => {
      console.log("\u2705 usedCitation incremented in Firestore for", planDocId);
      sendResponse({ success: true });
    }).catch((err) => {
      console.error("\u274C Failed to increment usedCitation:", err);
      sendResponse({ success: false, error: err.message });
    });
    return true;
  }
  if (message && message.type === "INCREMENT_CITATION_ACCUMULATION") {
    const userId = message.userId;
    if (!userId) {
      sendResponse({ success: false, error: "No userId provided" });
      return;
    }
    incrementCitationAccumulation(userId).then((result) => {
      if (result.success) {
        console.log("\u2705 Citation accumulation incremented successfully");
        sendResponse({ success: true });
      } else {
        console.error("\u274C Failed to increment citation accumulation:", result.error);
        sendResponse({ success: false, error: result.error });
      }
    }).catch((err) => {
      console.error("\u274C Error in citation accumulation:", err);
      sendResponse({ success: false, error: err.message });
    });
    return true;
  }
  if (message && message.type === "SAVE_CITATION_ACCUMULATION") {
    console.log("\u{1F4CA} [SAVE_CITATION_ACCUMULATION] Saving citation accumulation record...");
    const { userId, campaignID, domainName, status, index } = message;
    if (!userId) {
      console.error("\u274C [SAVE_CITATION_ACCUMULATION] No userId provided");
      sendResponse({ success: false, error: "No userId provided" });
      return;
    }
    let responseSent = false;
    const safeSendResponse = (response) => {
      if (!responseSent) {
        ...
        try {
          ...
          console.error("\u274C [SAVE_CITATION_ACCUMULATION] Error sending response:", error);
        }
      }
    };
    const currentDate = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    console.log(`\u{1F4CA} [SAVE_CITATION_ACCUMULATION] Data:`, { userId, campaignID, domainName, status, index, currentDate });
    (async () => {
      try {
        const citationData = {
          userId,
          ...
          // Step index when completed/aborted
          date: currentDate,
          createdAt: /* @__PURE__ */ new Date(),
          lastUpdated: /* @__PURE__ */ new Date()
        };
        console.log("\u{1F195} [SAVE_CITATION_ACCUMULATION] Creating new document with auto-generated ID");
        const citationAccumulationRef = collection(db, "citation_accumulation");
        const docRef = await addDoc(citationAccumulationRef, citationData);
        const docId = docRef.id;
        console.log(`\u2705 [SAVE_CITATION_ACCUMULATION] Document created successfully with ID: ${docId}`);
        safeSendResponse({
          success: true,
          accumulationId: docId,
          message: `Citation accumulation ${status || "saved"} successfully`
        });
      } catch (error) {
        console.error("\u274C [SAVE_CITATION_ACCUMULATION] Error saving citation accumulation:", error);
        console.error("\u274C [SAVE_CITATION_ACCUMULATION] Error details:", {
          code: error.code,
          message: error.message,
          stack: error.stack
        });
        safeSendResponse({
          success: false,
          error: error.message || "Unknown error saving citation accumulation"
        });
      }
    })();
    return true;
  ...
  if (message && message.type === "AUTH") {
    console.log("\u{1F511} Auth message received for user:", message.uid);
  }
  if (message && message.type === "CHECK_ELIGIBILITY") {
    console.log("\u{1F50D} Checking automation eligibility");
    chrome.storage.local.get(["isLoggedIn", "CampaignData", "plan_subscribers", "user_roles"], (result) => {
      if (chrome.runtime.lastError) {
        console.error("\u274C Error checking eligibility:", chrome.runtime.lastError);
        sendResponse({ eligible: false, reason: "Storage error", error: chrome.runtime.lastError.message });
        return;
      }
      const { isLoggedIn, CampaignData, plan_subscribers, user_roles } = result;
      console.log("\u{1F4CA} Eligibility check data:", {
        isLoggedIn,
        hasCampaignData: !!CampaignData,
        hasPlanSubscribers: !!plan_subscribers,
        hasUserRoles: !!user_roles,
        planStatus: plan_subscribers?.status,
        userRole: user_roles?.role,
        planDetails: plan_subscribers?.plans
      });
      if (!isLoggedIn) {
        console.log("\u274C Not eligible: User not logged in");
        sendResponse({ eligible: false, reason: "User not logged in" });
        return;
      }
      if (!CampaignData) {
        console.log("\u274C Not eligible: No CampaignData found");
        sendResponse({ eligible: false, reason: "No CampaignData found" });
        return;
      }
      let finalPlanSubscribers = plan_subscribers;
      if (!finalPlanSubscribers) {
        console.log("\u26A0\uFE0F plan_subscribers not found in chrome.storage.local, attempting to fetch from Firestore...");
        chrome.storage.local.get(["userId"], async (userIdResult) => {
          if (chrome.runtime.lastError) {
            console.error("\u274C Error getting userId:", chrome.runtime.lastError);
            console.log("\u274C Not eligible: No plan_subscribers found and cannot fetch from Firestore (no userId)");
            sendResponse({ eligible: false, reason: "No plan_subscribers found and cannot fetch from Firestore" });
            return;
          }
          const userId = userIdResult.userId;
          if (!userId) {
            console.error("\u274C No userId found in storage");
            console.log("\u274C Not eligible: No plan_subscribers found and no userId available");
            sendResponse({ eligible: false, reason: "No plan_subscribers found and no userId available" });
            return;
          }
          if (!auth.currentUser || auth.currentUser.uid !== userId) {
            console.error("\u274C User authentication mismatch or not authenticated");
            console.log("\u274C Not eligible: No plan_subscribers found and user authentication issue");
            sendResponse({ eligible: false, reason: "No plan_subscribers found and authentication issue" });
            return;
          }
          try {
            const planRef = doc(db, "plan_subscribers", userId);
            console.log(`\u{1F50D} [ELIGIBILITY] Fetching plan_subscribers from Firestore for user: ${userId}`);
            const planDocSnap = await getDoc(planRef);
            if (planDocSnap.exists()) {
              const planData = planDocSnap.data();
              console.log("\u2705 [ELIGIBILITY] plan_subscribers found in Firestore:", planData);
              chrome.storage.local.set({ plan_subscribers: planData }, () => {
                if (chrome.runtime.lastError) {
                  console.error("\u274C Error saving plan_subscribers to storage:", chrome.runtime.lastError);
                } else {
                  console.log("\u{1F4BE} [ELIGIBILITY] plan_subscribers saved to chrome.storage.local");
                }
              });
              finalPlanSubscribers = planData;
            } else {
              console.log(`\u26A0\uFE0F [ELIGIBILITY] No plan_subscribers document found in Firestore for user: ${userId}`);
              console.log("\u274C Not eligible: No plan_subscribers document exists in Firestore");
              sendResponse({
                eligible: false,
                reason: "No plan_subscribers document found in Firestore collection",
                details: `Document path: plan_subscribers/${userId} does not exist`
              });
              return;
            }
          } catch (error) {
            console.error("\u274C [ELIGIBILITY] Error fetching plan_subscribers from Firestore:", error);
            console.error("\u274C [ELIGIBILITY] Error details:", {
              code: error.code,
              message: error.message,
              stack: error.stack
            });
            console.log("\u274C Not eligible: No plan_subscribers found and Firestore fetch failed");
            sendResponse({
              eligible: false,
              reason: "No plan_subscribers found and Firestore fetch failed",
              error: error.message
            });
            return;
          }
          continueEligibilityCheck(finalPlanSubscribers, user_roles, CampaignData, sendResponse);
        });
        return true;
      }
      continueEligibilityCheck(finalPlanSubscribers, user_roles, CampaignData, sendResponse);
    });
    const continueEligibilityCheck = (plan_subscribers, user_roles, CampaignData, sendResponse2) => {
      console.log("\u{1F4CA} [ELIGIBILITY] Continuing eligibility check with plan_subscribers:", {
        hasPlanSubscribers: !!plan_subscribers,
        planStatus: plan_subscribers?.status,
        hasUserRoles: !!user_roles,
        userRole: user_roles?.role,
      ...
      if (!user_roles) {
        console.log("\u274C Not eligible: No user roles found");
        sendResponse2({ eligible: false, reason: "No user roles found" });
        return;
      }
      const userRole = user_roles.role;
      const allowedRoles = ["admin", "collaborator", "user"];
      if (!userRole || !allowedRoles.includes(userRole)) {
        console.log(`\u274C Not eligible: Invalid or missing user role (${userRole})`);
        sendResponse2({ eligible: false, reason: `Invalid user role: ${userRole}` });
        return;
      }
      const planStatus = plan_subscribers.status;
      if (planStatus !== "active") {
        console.log(`\u274C Not eligible: Plan not active (status: ${planStatus})`);
        sendResponse2({ eligible: false, reason: `Plan not active (status: ${planStatus})` });
        return;
      }
      const userPlan = plan_subscribers.plans;
      if (!userPlan) {
        console.log("\u274C Not eligible: No plan details found");
        sendResponse2({ eligible: false, reason: "No plan details found" });
        return;
      }
      const citationsUsed = parseInt(userPlan.usedCitation || "0", 10);
      const citations = parseInt(userPlan.citations || "0", 10);
      const remaining = Math.max(0, citations - citationsUsed);
      if (citationsUsed >= citations && citations > 0) {
        console.log(`\u274C Not eligible: Citation quota exhausted (${citationsUsed}/${citations})`);
        sendResponse2({
          eligible: false,
          reason: "Citation quota exhausted",
          usage: { citationsUsed, citations, remaining }
        });
        return;
      }
      console.log("\u2705 User is eligible for automation");
      sendResponse2({
        eligible: true,
        reason: "All checks passed",
        userRole,
        planDocId: user_roles.userId,
        // Add planDocId for citation increment
        usage: { citationsUsed, citations, remaining }
      });
    };
    return true;
  }
  if (message && message.type === "GET_USAGE_STATS") {
    console.log("\u{1F4CA} Getting usage statistics");
    chrome.storage.local.get(["plan_subscribers"], (result) => {
      if (chrome.runtime.lastError) {
        console.error("\u274C Error getting usage stats:", chrome.runtime.lastError);
        sendResponse({ error: chrome.runtime.lastError.message });
        return;
      }
      const { plan_subscribers } = result;
      if (!plan_subscribers || !plan_subscribers.plans) {
        sendResponse({ error: "No plan data found" });
        return;
      }
      const userPlan = plan_subscribers.plans;
      const citationsUsed = parseInt(userPlan.usedCitation || "0", 10);
      ...
      sendResponse({
        ...
        usage: {
          ...
          remaining,
          planStatus,
          isExceeded: citationsUsed >= citations && citations > 0
        }
      });
    });
    return true;
  }
  if (message && message.type === "GET_USER_ROLE_INFO") {
    console.log("\u{1F464} Getting user role information");
    chrome.storage.local.get(["user_roles", "isLoggedIn"], (result) => {
      if (chrome.runtime.lastError) {
        console.error("\u274C Error getting user role info:", chrome.runtime.lastError);
        sendResponse({ error: chrome.runtime.lastError.message });
        return;
      }
      const { user_roles, isLoggedIn } = result;
      if (!isLoggedIn) {
        sendResponse({ error: "User not logged in" });
        return;
      }
      if (!user_roles) {
        sendResponse({ error: "No user roles found" });
        return;
      }
      sendResponse({
        success: true,
        ...
        displayName: user_roles.displayName,
        lastUpdated: user_roles.lastUpdated,
        fullRoleData: user_roles
      });
    });
    return true;
  }
  if (message && message.type === "DEBUG_INDEXEDDB") {
    console.log("\u{1F50D} Debugging automation system state...");
    (async () => {
      try {
        const syncStatus = await automationSync.getSyncStatus();
        if (!syncStatus.indexedDBAvailable) {
          console.log("\u26A0\uFE0F IndexedDB is not available - checking Firestore directly");
          const collectionName = await getAutomationCollectionName();
          console.log(`\u{1F4DA} Debug: Using collection ${collectionName} for Firestore access`);
          console.log(`\u{1F3AD} Role-based collection selection: ${collectionName} (user role determines this)`);
          const automationTasksRef = collection(db, collectionName);
          const q2 = query(automationTasksRef, where("status", "==", "active"));
          const querySnapshot = await getDocs(q2);
          const firestoreData = querySnapshot.docs.map((doc2) => {
            const data = doc2.data();
            return {
              id: doc2.id,
              domainName: data.domainName,
          ...
          const allDocsQuery = query(automationTasksRef);
          const allDocsSnapshot = await getDocs(allDocsQuery);
          const allFirestoreData = allDocsSnapshot.docs.map((doc2) => {
            const data = doc2.data();
            return {
              id: doc2.id,
              domainName: data.domainName,
          ...
          const userRoleInfo = await new Promise((resolve) => {
            chrome.storage.local.get(["user_roles", "isLoggedIn"], (result) => {
              resolve(result);
            });
          });
          const debugInfo = {
            ...
            syncStatus,
            firestoreData,
            totalFirestoreTasks: firestoreData.length,
            allFirestoreData,
            totalAllFirestoreTasks: allFirestoreData.length,
            userRoleInfo,
            message: "IndexedDB not available - using direct Firestore access"
          };
          console.log("\u{1F4CA} Debug Info (Firestore only):", debugInfo);
        ...
        try {
          ...
          console.log("\u{1F4CA} IndexedDB Debug Info:", debugInfo);
          sendResponse({ success: true, debugInfo });
        } catch (indexedDBError) {
          console.error("\u274C IndexedDB access failed:", indexedDBError);
          const collectionName = await getAutomationCollectionName();
          console.log(`\u{1F4DA} Debug fallback: Using collection ${collectionName}`);
          console.log(`\u{1F3AD} Role-based collection selection: ${collectionName} (user role determines this)`);
          const automationTasksRef = collection(db, collectionName);
          const q2 = query(automationTasksRef, where("status", "==", "active"));
          const querySnapshot = await getDocs(q2);
          ...
          const debugInfo = {
            ...
            firestoreData,
            totalFirestoreTasks: firestoreData.length,
            indexedDBError: indexedDBError.message,
            message: "IndexedDB access failed - using Firestore fallback"
          };
          console.log("\u{1F4CA} Debug Info (Firestore fallback):", debugInfo);
          sendResponse({ success: true, debugInfo });
        }
      } catch (error) {
        console.error("\u274C Error debugging automation system:", error);
        sendResponse({
          success: false,
          error: error.message,
          message: "Debug operation failed completely"
        });
      }
    })();
    return true;
  }
  if (message && message.type === "GET_AUTOMATION") {
    const domainName = message.domainName;
    if (!domainName) {
      sendResponse({ success: false, error: "No domainName provided" });
      return;
    }
    chrome.storage.local.get(["indexedDBAvailable"], (result) => {
      if (chrome.runtime.lastError) {
        console.error("\u274C Error checking IndexedDB availability:", chrome.runtime.lastError);
        sendResponse({ success: false, error: "Storage error" });
        return;
      }
      const indexedDBAvailable = result.indexedDBAvailable !== false;
      if (!indexedDBAvailable) {
        console.log(`\u26A0\uFE0F IndexedDB not available for ${domainName}, using fallback`);
        sendResponse({ success: false, error: "IndexedDB not available - please refresh the extension" });
        return;
      }
      automationSync.getAutomationForDomain(domainName).then(async (automation) => {
        if (automation) {
          const collectionName = await getAutomationCollectionName();
          automation.sourceCollection = collectionName;
          console.log(`\u2705 Automation loaded for ${domainName} from collection: ${collectionName}`);
          console.log(`\u{1F39B}\uFE0F [GET_AUTOMATION] Controller requiredCategory: ${automation.controller?.requiredCategory !== false ? "true (default)" : "false"}`);
          console.log(`\u{1F39B}\uFE0F [GET_AUTOMATION] Controller object keys:`, automation.controller ? Object.keys(automation.controller) : "no controller");
          sendResponse({ success: true, automation });
        } else {
          console.log(`\u26A0\uFE0F No automation found for ${domainName}`);
          sendResponse({ success: false, error: "No automation found" });
        }
      }).catch((error) => {
        console.error(`\u274C Error loading automation for ${domainName}:`, error);
        sendResponse({ success: false, error: error.message });
  ...
  if (message && message.type === "SYNC_AUTOMATIONS") {
    let responseSent = false;
    const safeSendResponse = (response) => {
      if (!responseSent) {
        responseSent = true;
        try {
          ...
          console.error("\u274C [SYNC_AUTOMATIONS] Error sending response:", error);
        }
      }
    };
    automationSync.syncAllAutomations().then(() => {
      console.log("\u2705 [SYNC_AUTOMATIONS] IndexedDB sync completed");
      return automationSync.syncToChromeStorage();
    }).then(() => {
      console.log("\u2705 [SYNC_AUTOMATIONS] Chrome Storage sync completed");
      return dbManager.getActiveAutomationTasks();
    }).then((tasks) => {
      const domains = Array.from(new Set(tasks.map((t) => t.domainName)));
      chrome.storage.local.set({ automationDomains: domains }, () => {
        if (chrome.runtime.lastError) {
          console.warn("\u26A0\uFE0F Could not update automationDomains cache:", chrome.runtime.lastError);
        } else {
          console.log(`\u2705 [SYNC_AUTOMATIONS] Updated automationDomains cache (${domains.length} domains) for content script optimization`);
        }
      });
      console.log("\u2705 [SYNC_AUTOMATIONS] Manual automation sync completed (IndexedDB + Chrome Storage)");
      safeSendResponse({ success: true });
    }).catch((error) => {
      console.error("\u274C [SYNC_AUTOMATIONS] Manual automation sync failed:", error);
      safeSendResponse({ success: false, error: error.message });
    });
    setTimeout(() => {
      if (!responseSent) {
        console.warn("\u26A0\uFE0F [SYNC_AUTOMATIONS] Sync timeout - sending timeout response");
        safeSendResponse({ success: false, error: "Sync timeout" });
      }
    }, 1e4);
    return true;
  }
  if (message && message.type === "FORCE_SYNC_AND_CHECK") {
    console.log("\u{1F504} Force syncing and checking Firestore directly...");
    (async () => {
      try {
        await automationSync.syncAllAutomations();
        const collectionName = await getAutomationCollectionName();
        console.log(`\u{1F4DA} Force sync: Using collection ${collectionName}`);
        console.log(`\u{1F3AD} Role-based collection selection: ${collectionName} (user role determines this)`);
        const automationTasksRef = collection(db, collectionName);
        const q2 = query(automationTasksRef, where("status", "==", "active"));
        const querySnapshot = await getDocs(q2);
        ...
        console.log("\u{1F4CA} Firestore data after force sync:", firestoreData);
        sendResponse({
          success: true,
          firestoreData,
          totalFound: firestoreData.length
        });
      } catch (error) {
  ...
  if (message && message.type === "GET_SYNC_STATUS") {
    automationSync.getSyncStatus().then((status) => {
      console.log("\u{1F4CA} Sync status retrieved:", status);
  ...
  if (message && message.type === "VERIFY_DATA_INTEGRITY") {
    automationSync.verifyDataIntegrity().then((verification) => {
      if (verification) {
        console.log("\u2705 Data integrity verification completed:", verification);
  ...
  if (message && message.type === "GET_AUTOMATION_DETAILS") {
    const domainName = message.domainName;
    if (!domainName) {
      sendResponse({ success: false, error: "No domainName provided" });
      return;
    }
    automationSync.getAutomationDetails(domainName).then((details) => {
      if (details) {
        console.log(`\u2705 Automation details for ${domainName}:`, details);
        sendResponse({ success: true, details });
      } else {
        console.log(`\u26A0\uFE0F No automation details found for ${domainName}`);
        sendResponse({ success: false, error: "Automation not found", details });
      }
    }).catch((error) => {
      console.error(`\u274C Error getting automation details for ${domainName}:`, error);
      sendResponse({ success: false, error: error.message });
  ...
  if (message && message.type === "GET_ALL_AUTOMATION_DOMAINS") {
    dbManager.getActiveAutomationTasks().then((tasks) => {
      const domains = Array.from(new Set(tasks.map((t) => t.domainName)));
      console.log(`\u2705 Found ${domains.length} automation domains:`, domains);
      chrome.storage.local.set({ automationDomains: domains }, () => {
        if (chrome.runtime.lastError) {
          console.warn("\u26A0\uFE0F Could not update automationDomains cache:", chrome.runtime.lastError);
        } else {
          console.log("\u2705 Updated automationDomains cache for content script optimization");
        }
      });
      sendResponse({ success: true, domains });
    }).catch((error) => {
      console.error("\u274C Error getting automation domains:", error);
      sendResponse({ success: false, error: error.message });
    });
    return true;
  }
  if (message && message.type === "CLEAR_CACHE") {
    ...
    dbManager.clearCache(`automation_${domainName}`).then(() => {
      console.log(`\u2705 Cache cleared for ${domainName}`);
      sendResponse({ success: true });
    }).catch((error) => {
      console.error(`\u274C Error clearing cache for ${domainName}:`, error);
      sendResponse({ success: false, error: error.message });
    });
    return true;
  }
  if (message && message.type === "RESET_ALL_COMPLETION_STATUS") {
    console.log("\u{1F504} Resetting all automation completion statuses for all domains...");
    chrome.storage.local.get(null, (allData) => {
      const keysToRemove = [];
      const keysToReset = {};
      Object.keys(allData).forEach((key) => {
        if (key.startsWith("automationCompleted_")) {
          keysToRemove.push(key);
        } else if (key.startsWith("resumeIndex_")) {
          keysToReset[key] = 0;
        }
      });
      const totalKeys = keysToRemove.length + Object.keys(keysToReset).length;
      console.log(`\u{1F4CA} Found ${keysToRemove.length} completion keys and ${Object.keys(keysToReset).length} resume index keys to reset`);
      if (totalKeys === 0) {
        console.log("\u2139\uFE0F No completion or resume keys found to reset");
        sendResponse({ success: true, resetCount: 0, message: "No keys found to reset" });
        return;
      }
      if (keysToRemove.length > 0) {
        chrome.storage.local.remove(keysToRemove, () => {
          if (chrome.runtime.lastError) {
            console.error("\u274C Error removing completion keys:", chrome.runtime.lastError);
            sendResponse({ success: false, error: chrome.runtime.lastError.message });
            return;
          }
          console.log(`\u2705 Removed ${keysToRemove.length} completion keys`);
          if (Object.keys(keysToReset).length > 0) {
            chrome.storage.local.set(keysToReset, () => {
              if (chrome.runtime.lastError) {
                console.error("\u274C Error resetting resume index keys:", chrome.runtime.lastError);
                sendResponse({ success: false, error: chrome.runtime.lastError.message });
                return;
              }
              console.log(`\u2705 Reset ${Object.keys(keysToReset).length} resume index keys to 0`);
              console.log(`\u2705 Successfully reset completion status for all domains (${totalKeys} total keys)`);
              sendResponse({
                success: true,
                resetCount: totalKeys,
                completionKeysRemoved: keysToRemove.length,
                resumeKeysReset: Object.keys(keysToReset).length,
                message: `Reset ${totalKeys} keys across all domains`
              });
            });
          } else {
            console.log(`\u2705 Successfully reset completion status for all domains (${keysToRemove.length} completion keys removed)`);
            sendResponse({
              success: true,
              resetCount: keysToRemove.length,
              completionKeysRemoved: keysToRemove.length,
              resumeKeysReset: 0,
              message: `Reset ${keysToRemove.length} completion keys`
            });
          }
        });
      } else if (Object.keys(keysToReset).length > 0) {
        chrome.storage.local.set(keysToReset, () => {
          if (chrome.runtime.lastError) {
            console.error("\u274C Error resetting resume index keys:", chrome.runtime.lastError);
            sendResponse({ success: false, error: chrome.runtime.lastError.message });
          ...
          sendResponse({
            success: true,
            resetCount: Object.keys(keysToReset).length,
            completionKeysRemoved: 0,
            resumeKeysReset: Object.keys(keysToReset).length,
            message: `Reset ${Object.keys(keysToReset).length} resume index keys`
          });
        });
      }
    });
  ...
  if (message && message.type === "GET_INDEXEDDB_STATS") {
    Promise.all([
      dbManager.getAllAutomationTasks(),
      dbManager.getAllVersions(),
      dbManager.getActiveAutomationTasks(),
      dbManager.getAllConsolidationSetups()
    ]).then(([allTasks, allVersions, activeTasks, consolidationSetups]) => {
      const stats = {
        totalTasks: allTasks.length,
        activeTasks: activeTasks.length,
        totalVersions: allVersions.length,
        totalConsolidationSetups: consolidationSetups.length,
        domains: Array.from(new Set(allTasks.map((t) => t.domainName))),
        totalSteps: allTasks.reduce((sum, t) => sum + (t.steps?.length || 0), 0),
        averageStepsPerTask: allTasks.length > 0 ? allTasks.reduce((sum, t) => sum + (t.steps?.length || 0), 0) / allTasks.length : 0,
        lastModified: allTasks.length > 0 ? new Date(Math.max(...allTasks.map((t) => t.lastModified.getTime()))) : null
      };
      console.log("\u{1F4CA} IndexedDB statistics:", stats);
  ...
  if (message && message.type === "GET_CONSOLIDATION_SETUP") {
    const domainName = message.domainName;
    if (!domainName) {
      sendResponse({ success: false, error: "No domainName provided" });
      return true;
    ...
    const safeSendResponse = (response) => {
      if (!responseSent) {
        ...
        try {
          ...
          console.error("\u274C [GET_CONSOLIDATION_SETUP] Error sending response:", error);
        }
      }
    };
    console.log(`\u{1F50D} [GET_CONSOLIDATION_SETUP] Requested for domain: ${domainName}`);
    automationSync.getConsolidationSetupForDomain(domainName).then((setup) => {
      if (setup) {
        console.log(`\u2705 [GET_CONSOLIDATION_SETUP] Consolidation setup loaded for ${domainName}`);
        console.log(`\u{1F4CA} [GET_CONSOLIDATION_SETUP] Config keys: ${Object.keys(setup.config || {}).join(", ")}`);
        safeSendResponse({ success: true, setup });
      } else {
        console.log(`\u26A0\uFE0F [GET_CONSOLIDATION_SETUP] No consolidation setup found for ${domainName}`);
        console.log(`\u{1F4A1} [GET_CONSOLIDATION_SETUP] This is normal if no setup exists in Firestore for this domain`);
        safeSendResponse({ success: false, error: "No consolidation setup found" });
      }
    }).catch((error) => {
      console.error(`\u274C [GET_CONSOLIDATION_SETUP] Error loading consolidation setup for ${domainName}:`, error);
      console.error(`\u274C [GET_CONSOLIDATION_SETUP] Error details:`, {
        message: error.message,
        stack: error.stack,
        name: error.name
      });
    ...
    setTimeout(() => {
      if (!responseSent) {
        console.warn(`\u26A0\uFE0F [GET_CONSOLIDATION_SETUP] Timeout - sending timeout response`);
        safeSendResponse({ success: false, error: "Request timeout" });
      }
    }, 1e4);
    return true;
  }
  if (message && message.type === "SYNC_CONSOLIDATION_SETUPS") {
    automationSync.syncConsolidationSetups().then(() => {
      console.log("\u2705 Manual consolidation setup sync completed");
      sendResponse({ success: true });
    }).catch((error) => {
      console.error("\u274C Manual consolidation setup sync failed:", error);
      sendResponse({ success: false, error: error.message });
    });
    return true;
  }
  if (message && message.type === "RECOVER_AUTH") {
    console.log("\u{1F504} Manual authentication recovery requested");
    try {
      handleAuthRecovery();
      sendResponse({ success: true, message: "Authentication recovery initiated" });
    } catch (error) {
      console.error("\u274C Error during manual auth recovery:", error);
      sendResponse({ success: false, error: error instanceof Error ? error.message : String(error) });
    }
    return true;
  }
  if (message && message.type === "TEST_FIRESTORE_ACCESS") {
    console.log("\u{1F50D} Manual Firestore access test requested");
    testFirestoreAccess().then((result) => {
      console.log("\u{1F50D} Firestore access test result:", result);
      sendResponse({ success: true, result });
    }).catch((error) => {
      console.error("\u274C Error during Firestore access test:", error);
      sendResponse({ success: false, error: error instanceof Error ? error.message : String(error) });
    });
    return true;
  }
  if (message && message.type === "FORCE_AUTH_RECOVERY") {
    console.log("\u{1F504} Manual authentication recovery requested");
    try {
      handleAuthRecovery();
      sendResponse({ success: true, message: "Authentication recovery initiated" });
  ...
  if (message && message.type === "GET_AUTH_STATUS") {
    console.log("\u{1F50D} Getting detailed authentication status");
    const authStatus = {
      ...
      tokenError: null,
      listeners: {
        campaignData: !!unsubscribeCampaignData,
        planSubscriber: !!unsubscribePlanSubscriber,
        userRole: !!unsubscribeUserRole
      },
      storage: {
        isLoggedIn: false,
        campaignDataStatus: null,
    ...
    if (auth.currentUser) {
      auth.currentUser.getIdToken(false).then((token) => {
        authStatus.hasValidToken = !!token;
        console.log("\u2705 Token check completed");
      }).catch((error) => {
        authStatus.tokenError = error.message;
    ...
    chrome.storage.local.get([
      ...
      authStatus.storage = {
        ...
        campaignDataStatus: result.CampaignDataStatus || null,
        hasCampaignData: !!result.CampaignData,
        hasPlanSubscribers: !!result.plan_subscribers,
        hasUserRoles: !!result.user_roles
      };
      console.log("\u{1F4CA} Authentication status:", authStatus);
      sendResponse({ success: true, authStatus });
    });
    return true;
  }
  if (message && message.type === "CAPTURE_VISIBLE_TAB") {
    console.log("\u{1F4F8} Capturing visible tab screenshot...");
    chrome.windows.getCurrent((window2) => {
      ...
      if (windowId === void 0) {
        ...
        sendResponse({
          error: "Failed to capture tab",
          details: "No window ID available"
        });
        return;
      }
  ...
  if (message && message.type === "DOWNLOAD_SCREENSHOT") {
    console.log("\u{1F4BE} Downloading screenshot...");
    const { dataUrl, filename } = message;
    if (!dataUrl) {
      console.error("\u274C No dataUrl provided for download");
      sendResponse({
        success: false,
    ...
    try {
      ...
      if (!dataUrl.startsWith("data:")) {
        downloadUrl = `data:image/png;base64,${dataUrl}`;
      }
      chrome.downloads.download({
        ...
        console.log("\u2705 Screenshot downloaded successfully:", downloadId);
        sendResponse({ success: true, downloadId });
      });
    } catch (error) {
      console.error("\u274C Error processing screenshot download:", error);
      sendResponse({
        success: false,
...
async function initialize() {
  try {
    console.log("\u{1F680} Starting background script initialization...");
    setupAuthListener();
    console.log("\u2705 Auth listener initialized");
    try {
      await automationSync.initialize();
      console.log("\u2705 IndexedDB and automation sync initialized");
    } catch (indexedDBError) {
      console.error("\u274C IndexedDB initialization failed:", indexedDBError);
      console.log("\u26A0\uFE0F Continuing without IndexedDB - will use fallback methods");
      chrome.storage.local.set({ indexedDBAvailable: false }, () => {
        if (chrome.runtime.lastError) {
          console.error("\u274C Error setting IndexedDB flag:", chrome.runtime.lastError);
        }
    ...
    chrome.storage.local.set({ backgroundInitError: error.message }, () => {
      if (chrome.runtime.lastError) {
        console.error("\u274C Error setting init error flag:", chrome.runtime.lastError);
      }
    });
  }
}
